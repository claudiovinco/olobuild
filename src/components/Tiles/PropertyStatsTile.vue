  ...
  <div class="mb-font-sans">
    <div :style="gridStyle">
      <div v-for="stat in stats" :key="stat.label" :style="statStyle">
        ...
        <div :style="{ fontSize:'13px', fontWeight:'600', color:'var(--olo-color-text-muted, #6b7280)', textTransform:'uppercase', letterSpacing:'0.5px' }">{{ stat.label }}</div>
      </div>
    </div>
  </div>
</template>
...
const defaults = { columns: 3, gap: 24, number_size: 36, accent_color: '', bg: '', border_radius: 12, padding: 28 };
...
const statStyle = computed(() => ({ background: s.value.bg || 'var(--olo-color-surface-alt, #f6f7f9)', borderRadius: s.value.border_radius+'px', padding: s.value.padding+'px', textAlign:'center' }));
</script>
