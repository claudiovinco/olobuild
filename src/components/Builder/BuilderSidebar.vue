<template>
  <div class="mb-w-60 mb-bg-gray-800 mb-border-r mb-border-gray-700 mb-overflow-y-auto mb-shrink-0">
    ...
    <div v-if="activeTab === 'tiles'" class="tp-root">
      <!-- Search filter -->
      <div class="tp-search">
        <svg class="tp-search-icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
        <input
        ...
        />
        <button v-if="sidebarSearch" class="tp-search-clear" @click="sidebarSearch = ''">&#215;</button>
      </div>
...
const tilesByCategory = computed(() => {
  const raw = tilesStore.tilesByCategory;
  const q = sidebarSearch.value.trim().toLowerCase();
  if (!q) return raw;
  const filtered = {};
  for (const [cat, tiles] of Object.entries(raw)) {
    const matched = tiles.filter(t => t.name.toLowerCase().includes(q) || t.type.toLowerCase().includes(q));
...
const favoriteTilesList = computed(() => {
  ...
  const q = sidebarSearch.value.trim().toLowerCase();
  const favs = all.filter(t => favorites.value.has(t.type));
  if (!q) return favs;
  return favs.filter(t => t.name.toLowerCase().includes(q) || t.type.toLowerCase().includes(q));
});
...
const recentTilesList = computed(() => {
  ...
  const list = recentTypes.value.map(type => map[type]).filter(Boolean);
  const q = sidebarSearch.value.trim().toLowerCase();
  if (!q) return list;
  return list.filter(t => t.name.toLowerCase().includes(q) || t.type.toLowerCase().includes(q));
});
