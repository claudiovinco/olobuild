...
/**
 * Tile Overlay Slider — split CONTENUTO/STILE (regola universale Olobuild).
 *   fields[]      → slides (content-items), colonne (responsive), proporzione/altezza/adattamento immagine,
 *                   posizione overlay, dimensione titolo, hover_effect/hover_overlay (comportamento),
 *                   visibilità frecce/dots, visibilità CTA + testo CTA, posizione ribbon
 *   styleFields[] → preset, sfondo, tipografia, tweak effetto preset, gap, allineamento testo,
 *                   stile overlay (primary/default), padding overlay, raggio slide,
...
export default {
  ...
  fields: [
    ...
    { key: 'show_dots', label: t('Mostra indicatori'), type: 'toggle' },

    { type: 'separator', label: t('Call to action') },
    { key: 'show_cta', label: t('Mostra CTA su ogni slide'), type: 'toggle' },
  ...
  styleFields: [
    ...
    { key: 'preset', label: t('Stile'), type: 'select', options: [
      { value: 'cinematic-overlay', label: t('Cinematic Overlay (default)') },
      { value: 'minimal-caption',   label: t('Minimal Caption (testo sotto)') },
      { value: 'magazine-cover',    label: t('Magazine Cover (editorial)') },
      { value: 'split-frame',       label: t('Split Frame (cornice doppia)') },
      { value: 'gradient-bottom',   label: t('Gradient Bottom (gradient soft)') },
    ...
    { key: 'effect_color', label: t('Colore effetto'), type: 'color',
      description: t('Colore principale degli accent del preset.'),
    ...
    { key: 'slide_radius', label: t('Arrotondamento slide (px)'), type: 'range', min: 0, max: 40, step: 1 },
