...
export default {
  ...
  fields: [
    ...
    { key: 'min', label: t('Valore minimo'), type: 'number', min: 0, step: 1 },
    { key: 'max', label: t('Valore massimo'), type: 'number', min: 0, step: 1 },
    { key: 'step', label: t('Incremento (step)'), type: 'number', min: 1, step: 1 },
    { key: 'value', label: t('Valore iniziale'), type: 'number', min: 0, step: 1 },
    { key: 'rate', label: t('Tasso annuo (0 = lineare)'), type: 'number', min: 0, max: 1, step: 0.01,
      description: t('0 → valore × anni (costo/ore/budget). >0 → montante composto (es. 0.06 = 6%).') },
    { key: 'years', label: t('Anni / moltiplicatore'), type: 'number', min: 1, max: 100, step: 1 },
