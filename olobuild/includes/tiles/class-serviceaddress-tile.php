...
class Olo_ServiceAddress_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        $icon_svg = '<svg width="' . ( $font_size + 4 ) . '" height="' . ( $font_size + 4 ) . '" viewBox="0 0 24 24" fill="none">'
            . '<path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z" stroke="' . esc_attr( $s['icon_color'] ) . '" stroke-width="1.8" fill="' . esc_attr( $s['icon_color'] ) . '" fill-opacity="0.15"/>'
            . '<circle cx="12" cy="9" r="2.5" stroke="' . esc_attr( $s['icon_color'] ) . '" stroke-width="1.8" fill="' . esc_attr( $s['icon_color'] ) . '" fill-opacity="0.3"/>'
            . '</svg>';
        ...
        <div style="display:flex;align-items:flex-start;gap:10px;<?php echo implode( ';', $wrap_styles ); ?>">
            ...
            <div style="display:flex;flex-direction:column;gap:3px">
                <?php if ( ! empty( $s['show_label'] ) ) : ?>
                    <span style="font-size:<?php echo $font_size - 3; ?>px;color:<?php echo esc_attr( $s['label_color'] ); ?>;font-weight:600;text-transform:uppercase;letter-spacing:0.5px"><?php echo esc_html( $s['label_text'] ); ?></span>
                <?php endif; ?>
                <?php if ( $address ) : ?>
                    <span style="font-size:<?php echo $font_size; ?>px;color:<?php echo esc_attr( $s['text_color'] ); ?>;line-height:1.4"><?php echo esc_html( $address ); ?></span>
                <?php endif; ?>
                <?php if ( ! empty( $s['show_locality'] ) && $locality ) : ?>
                    <span style="font-size:<?php echo $font_size - 1; ?>px;color:<?php echo esc_attr( $s['label_color'] ); ?>"><?php echo esc_html( $locality ); ?></span>
                <?php endif; ?>
                <?php if ( ! empty( $s['show_map_link'] ) && $map_url ) : ?>
                    <a href="<?php echo esc_url( $map_url ); ?>" target="_blank" rel="noopener" style="font-size:<?php echo $font_size - 2; ?>px;color:<?php echo esc_attr( $s['link_color'] ); ?>;text-decoration:none;display:inline-flex;align-items:center;gap:4px;margin-top:4px">
