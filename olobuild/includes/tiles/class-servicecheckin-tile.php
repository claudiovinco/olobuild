...
class Olo_ServiceCheckin_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        <div class="<?php echo esc_attr( $uid ); ?>" style="<?php echo $style; ?>">
            <?php if ( $checkin_time ) : ?>
                <strong style="color:<?php echo esc_attr( $s['label_color'] ); ?>"><?php echo esc_html( $t['checkin_label'] ); ?></strong> <?php echo esc_html( $t['checkin_prefix'] . $checkin_time ); ?>
            <?php endif; ?>
            <?php if ( $checkout_time ) : ?>
                &nbsp;| &nbsp;<strong style="color:<?php echo esc_attr( $s['label_color'] ); ?>"><?php echo esc_html( $t['checkout_label'] ); ?></strong> <?php echo esc_html( $t['checkout_prefix'] . $checkout_time ); ?>
            <?php endif; ?>
            <?php if ( $min_stay ) : ?>
                <br><strong style="color:<?php echo esc_attr( $s['label_color'] ); ?>"><?php echo esc_html( $t['minstay_label'] ); ?></strong> <?php echo esc_html( $min_stay ); ?> <?php echo esc_html( (int) $min_stay === 1 ? $t['night_single'] : $t['night_plural'] ); ?>
            <?php endif; ?>
        </div>
        <?php
        return ob_get_clean();
