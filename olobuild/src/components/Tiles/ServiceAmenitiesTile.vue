<template>
  <div class="mb-font-sans">
    <h3 v-if="s.show_title" :style="{ fontSize: s.title_size+'px', fontWeight:'700', color: s.title_color, margin:'0 0 12px', fontStyle:'italic' }">
      {{ s.title_text }}
    </h3>
    <div :style="gridStyle">
      <div v-for="amenity in amenities" :key="amenity"
           :style="{ display:'flex', alignItems:'center', gap:'6px', fontSize: s.text_size+'px', color: s.text_color, padding:'3px 0' }">
...
const defaults = {
  ...
  title_size: '18', title_color: '#1F2937', check_color: '', text_color: '#374151', text_size: '14',
};
const s = computed(() => ({ ...defaults, ...props.settings }));
const accent = computed(() => s.value.check_color || '#6366F1');
