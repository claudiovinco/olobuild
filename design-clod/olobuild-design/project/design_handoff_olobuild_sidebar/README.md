...
- Tab bar in alto (38px) con due tab: `Elementi` (icona grid) / `Struttura` (icona layers). Tab attiva = bordo inferiore 2px verde brand `#4a8c2a`.
- Body: grid `64px 1fr`.
...
- **Pannello (resto)**:
  ...
  - Hover card: bordo verde brand, bg `primary-50`, icon box bianco con shadow, traslate -1px, grip drag a destra.
...
- **Pannello**:
  ...
  - **Selezionato** (es. nodo Immagine in Body): bg `primary-50` + box-shadow `inset 3px 0 0 #4a8c2a` + icona verde + tag pill `selezionato` verde.
  - **Drop zone vuota**: riga separata con bordo dashed `primary-200`, bg `primary-50`, label "Trascina qui" centrata, icona `+`.
- **Breadcrumb sticky in basso**: bg `bg-soft`, top border. Eyebrow "Selezionato" + path `Body › Sezione › Row › Column › 🖼 Immagine` (ultimo nodo come pill verde).
...
- **Drag start su card/riga elemento**: `dataTransfer.setData('text/plain', label)` + `setDragImage` con un mini ghost custom (white card + bordo verde + label, shadow `0 8px 20px rgba(74,140,42,.25)`). Cursor `grab → grabbing`.
- **Drop zone canvas**: bordo box-shadow verde + hint pill "Rilascia per inserire" sul fondo durante il drag. Implementato con `dragover/dragleave/drop`.
...
**Colori brand**:
- Verde primario `#4a8c2a` (`--ot-primary`)
- Verde brillante `#3fa23f` (`--ot-primary-bright`)
- Tints: `primary-50 #f0f7ec`, `primary-100 #dcefd2`, `primary-200 #b9dfaa`, `primary-300 #8fcb78`, `primary-dark #2d722d`
