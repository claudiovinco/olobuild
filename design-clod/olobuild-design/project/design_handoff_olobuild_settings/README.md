...
3. **Tabs primarie** — pill container `border 1px / radius 8px / padding 2px`, 3 tab (`Contenuto`, `Stile`, `Avanzate`). Attivo: bg `#4a8c2a` + testo bianco + ombra `0 1px 3px rgba(74,140,42,.3)`. Inattivo: trasparente + testo muted.
...
- Stato attivo: bg `#fff`, **barra accent 2px verde** sul **lato destro** (`border-radius: 2px 0 0 2px`), label nera, dot 6px (`primary`) come indicatore.
...
- Tabs primarie active: `bg --ot-primary` + ombra `0 1px 3px rgba(74,140,42,.3)`.
