<template>
  <div class="mb-relative mb-overflow-hidden" :style="containerStyle">
    ...
    <!-- Video badge (preview only) -->
    <div
      v-if="s.bg_type === 'video'"
    ...
    <div class="mb-relative mb-flex mb-w-full" :style="contentFlexStyle">
      <div :style="contentBlockStyle">
        <h1 class="mb-text-3xl mb-font-bold mb-mb-3" v-html="s.title || 'Hero Title'"></h1>
        <div class="mb-text-lg mb-opacity-90 mb-mb-6" v-html="s.subtitle || 'Subtitle goes here'"></div>
...
const containerStyle = computed(() => {
  ...
  if (bgType === 'color') {
    st.backgroundColor = s.value.bg_color || '#6366F1';
  } else if (bgType === 'gradient') {
    const angle = parseInt(s.value.bg_gradient_angle) || 135;
    st.background = `linear-gradient(${angle}deg, ${s.value.bg_gradient_from || '#6366F1'}, ${s.value.bg_gradient_to || '#8B5CF6'})`;
  } else if (bgType === 'image' && !s.value.bg_image) {
    st.backgroundColor = '#1f2937';
  } else if (bgType === 'video') {
    st.backgroundColor = '#111827';
  }
...
function ctaStyle(n) {
  ...
  const radius = (parseInt(s.value.cta_radius) || 6) + 'px';
  ...
  if (n === 1) {
    ...
    if (explicit) {
      ...
      textColor = s.value.bg_color || '#6366F1';
    } else {
