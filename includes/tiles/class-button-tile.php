...
class Olobuild_Button_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        // Typography preset (global style) — slug whitelist: il valore finisce
        ...
        $shadow = Olobuild_Tile_Utils::shadow_value( $s, 'shadow', 'button' );
        ...
        $hover_shadow = ( ( $s['hover_shadow'] ?? '' ) !== '' ) ? Olobuild_Tile_Utils::shadow_value( $s, 'hover_shadow', 'button' ) : '';
        ...
        ob_start();
        ?>
        <?php // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above: safe_color_css() whitelist for every colour, absint()/floatval() for sizes, in_array() whitelists for enums, slug-whitelisted typography preset, Olobuild_CSS_Builder/Olobuild_Tile_Utils/Olobuild_Tile_Base shared helpers (sanitized internally), fixed literals from ternaries/switch maps and the internally generated $uid. ?>
        <style>
        ...
        <?php if ( $border_css ) echo "{$btn_sel}{{$border_css}}"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base::build_border_css() (integer-forced widths) for the internally generated uid ?>
        <?php echo $border_effect_css; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by the Olobuild_Tile_Base::build_border_effect_css() shared helper ?>
        </style><?php endif; ?>
        ...
        <div class="olo-button <?php echo esc_attr( $align_class ); ?> <?php echo esc_attr( $uid ); ?>" style="padding: 16px 0; overflow: visible;">
            ...
            if ( $has_hover_media ) {
                echo $this->render_hover_wrap( $btn_html, $s['hover_image'] ?? '', $s['hover_video'] ?? '' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $btn_html assembled above from esc_url()/esc_attr()/esc_html()'d parts; render_hover_wrap() escapes hover media URLs internally
            } else {
                echo $btn_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- assembled above from esc_url()/esc_attr()/esc_html()'d parts, absint()'d spacing, internally escaped tfx fragments and Olobuild_CSS_Builder bg markup (escaped internally)
            }
            ?>
        </div>
        <?php
