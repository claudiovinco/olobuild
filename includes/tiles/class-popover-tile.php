...
class Olobuild_Popover_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        ob_start();
        ?>
        <?php // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above: colours via the safe_color_css() whitelist, integers via absint(), radius via build_border_radius_css()/Olobuild_Tile_Utils::radius_force_css() (integer px), hover effect from fixed-literal branches; $uid is internally generated. ?>
        <style>
        ...
        <div class="olo-popover uk-inline <?php echo esc_attr( $uid ); ?>" style="width:100%;">
            <?php if ( ! empty( $s['image'] ) ) : ?>
                <?php echo Olobuild_Tile_Utils::img_srcset( absint( $s['image_id'] ?? 0 ), $s['image'], $s['image_alt'] ?? '', '', 'full', 'style="' . esc_attr( $img_style ) . '"' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- <img> markup built by Olobuild_Tile_Utils::img_srcset() with esc_url()/esc_attr() internally ?>
            <?php else : ?>
        ...
        <?php // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- static inline script; $uid is internally generated and esc_js()-escaped. ?>
        <script>
        (function () {
            var root = document.querySelector('.<?php echo esc_js( $uid ); ?>');
            if (!root) { return; }
            root.querySelectorAll('.olo-popover-marker[aria-controls]').forEach(function (trigger) {
                ...
                if (label) { drop.setAttribute('aria-label', label); }
                drop.addEventListener('show', function () { trigger.setAttribute('aria-expanded', 'true'); });
                drop.addEventListener('hide', function () { trigger.setAttribute('aria-expanded', 'false'); });
            });
        })();
        </script>
        <?php // phpcs:enable WordPress.Security.EscapeOutput.OutputNotEscaped ?>
