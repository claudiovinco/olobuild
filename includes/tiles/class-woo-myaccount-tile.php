...
class Olo_Woo_Myaccount_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        $sb_act_bg = $this->safe_color_css( $s['sidebar_active_bg'] ) ?: 'var(--olo-color-primary, #e1474f)';
        $sb_act_c  = $this->safe_color_css( $s['sidebar_active_color'] ) ?: 'var(--olo-color-on-primary, #ffffff)';
        $sb_c      = $this->safe_color_css( $s['sidebar_color'] ) ?: 'var(--olo-color-text, #1f2937)';
        $cnt_bg    = $this->safe_color_css( $s['content_bg'] ) ?: 'var(--olo-color-surface, #ffffff)';
        $h_color   = $this->safe_color_css( $s['heading_color'] ) ?: 'var(--olo-color-text, #374151)';
        $t_color   = $this->safe_color_css( $s['text_color'] ) ?: 'var(--olo-color-text, #1f2937)';
        ...
        ob_start();
        ?>
        <?php // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above: every colour via the safe_color_css() whitelist (or fixed var() fallback literals), radius via the absint()-based Olo_Tile_Utils::border_radius()/radius_int()/radius_force_css() helpers, layout via in_array() whitelist; $uid is internally generated. ?>
        <style>
        ...
        <div id="<?php echo esc_attr( $uid ); ?>" class="olo-woo-myaccount">
            ...
            echo do_shortcode( '[woocommerce_my_account]' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- WooCommerce my-account shortcode output (core/Woo templates escape their own output)
            ?>
        </div>
        <?php
                // Border system
