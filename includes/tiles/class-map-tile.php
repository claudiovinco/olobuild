...
class Olobuild_Map_Tile extends Olobuild_Tile_Base {
    ...
    private function render_dynamic_service( $s ) {
         ...
         *
         * Questo blocco stava in fondo a build_multi_enhance_js(), dove ne' $s
         * ne' $uid esistono: leggeva null, quindi il bordo configurato dal
         * docente non compariva mai. Ed era anche il posto sbagliato per un
         * altro motivo: quel metodo restituisce una STRINGA di script, quindi
         * il <style> sarebbe finito dentro il codice JavaScript invece che
         * nella pagina.
         */
    ...
    private function render_locations( $s ) {
        ...
        $view_mode     = in_array( $s['view_mode'] ?? 'list', [ 'list', 'grid' ], true ) ? ( $s['view_mode'] ?? 'list' ) : 'list';
        $sort_default  = in_array( $s['sort_default'] ?? 'default', [ 'default', 'title_asc', 'title_desc', 'newest', 'distance' ], true ) ? ( $s['sort_default'] ?? 'default' ) : 'default';
