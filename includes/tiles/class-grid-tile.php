...
class Olobuild_Grid_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
                ...
                <div<?php if ( $tag_class_str ) : ?> class="<?php echo esc_attr( $tag_class_str ); ?>"<?php endif; ?>>
                    <<?php echo $link_tag; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- fixed a/div tag from ternary; $link_attrs is built above from esc_url() and fixed literals ?> class="olo-grid-card olo-grid-card--<?php echo esc_attr( $card_style ); ?><?php echo $card_hover !== 'none' ? ' olo-grid-hover--' . esc_attr( $card_hover ) : ''; ?>"<?php echo $link_attrs; ?>>
                        ...
                        if ( ! empty( $item['image'] ) ) {
                            ...
                            echo $this->render_hover_wrap( $img_html, $item['hover_image'] ?? '', $item['hover_video'] ?? '' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- markup built by Olobuild_Tile_Base::render_hover_wrap() from the esc_attr()/esc_url()'d $img_html above; hover URLs are escaped inside the helper
        ...
        if ( $tfx_css ) echo '<style>' . $tfx_css . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Text_Effects::css() from whitelisted colours and intval()'d delays
