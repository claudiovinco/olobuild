...
class Olobuild_Woo_Product_Bundle_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        if ( empty( $raw_ids ) ) {
            return '<div style="padding:30px;text-align:center;color:var(--olo-color-text-muted, #9CA3AF);font-size:14px">'
                 . esc_html( olobuild_t( 'Nessun prodotto nel bundle. Inserisci gli ID dei prodotti nelle impostazioni.' ) )
                 . '</div>';
        }
        ...
        if ( empty( $products ) ) {
            return '<div style="padding:30px;text-align:center;color:var(--olo-color-text-muted, #9CA3AF);font-size:14px">'
                 . esc_html( olobuild_t( 'Nessun prodotto valido nel bundle.' ) )
                 . '</div>';
        }
        ...
        ob_start();
        // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above (safe_color_css(), absint(), Olobuild_Tile_Utils::border_radius()/radius_force_css(), fixed literal ternaries) and the internally generated $uid.
        ?>
        <style>
            .<?php echo $uid; ?> {
...
<?php // phpcs:enable WordPress.Security.EscapeOutput.OutputNotEscaped -- column 0 + closing tag so this line emits zero bytes ?>
        ...
        <div class="<?php echo esc_attr( $uid ); ?>">
                ...
                <div class="<?php echo esc_attr( $uid ); ?>-item">
                    ...
                    <div class="<?php echo esc_attr( $uid ); ?>-item-img">
                        ...
                        <a href="<?php echo esc_url( $p['url'] ); ?>">
                            <?php echo wp_get_attachment_image( $p['image_id'], 'woocommerce_thumbnail' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- <img> markup generated by WordPress core wp_get_attachment_image() ?>
                        </a>
                        <?php else : ?>
            ...
            <div class="<?php echo esc_attr( $uid ); ?>-summary">
                <div class="<?php echo esc_attr( $uid ); ?>-price-row">
                    <span><?php echo esc_html( olobuild_t( 'Prezzo totale' ) ); ?></span>
                    <span class="olo-bundle-val"><?php echo wc_price( $total_price ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- price HTML generated by WooCommerce wc_price() on a float ?></span>
                </div>

                <?php if ( ! empty( $s['show_savings'] ) ) : ?>
                <?php if ( $discount_pct > 0 ) : ?>
                <div class="<?php echo esc_attr( $uid ); ?>-savings">
                    <span><?php echo esc_html( olobuild_t( 'Risparmi' ) ); ?> (<?php echo absint( $discount_pct ); ?>%)</span>
                    <span>-<?php echo wc_price( $discount_amount ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- price HTML generated by WooCommerce wc_price() on a float ?></span>
                </div>
                <?php endif; ?>
                <?php endif; ?>

                <div class="<?php echo esc_attr( $uid ); ?>-total">
                    <span><?php echo esc_html( olobuild_t( 'Prezzo bundle' ) ); ?></span>
                    <span><?php echo wc_price( $bundle_price ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- price HTML generated by WooCommerce wc_price() on a float ?></span>
                </div>

                <button type="button" class="<?php echo esc_attr( $uid ); ?>-atc-btn" data-olo-bundle-atc><?php echo $btn_text; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped via esc_html() at assignment above ?></button>
            </div>
        </div>
        ...
        (function(){
            ...
            atcBtn.addEventListener('click', function(){
                ...
                atcBtn.textContent = '<?php echo esc_js( olobuild_t( 'Aggiunta in corso...' ) ); ?>';
