...
class Olobuild_List_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        $pad = Olobuild_Tile_Utils::spacing_css( $s['tile_padding'] ?? $s['padding'] ?? 16, 16 );
            ...
            ?>
                ...
                <li style="<?php echo $li_style; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- assembled from absint()'d gap/spacing and fixed CSS literals ?>"><?php echo $inner_open; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- opening <a> assembled above from fixed markup, esc_url()'d link and absint()'d gap ?>
                    ...
                    <?php else : ?>
                        ...
                        <?php if ( $icon === 'x' ) : ?>
                        <span class="olo-list-sr-only" style="position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0;"><?php echo esc_html( olobuild_t( 'Non incluso' ) ); ?></span>
                        <?php endif; ?>
                    <?php endif; ?>
