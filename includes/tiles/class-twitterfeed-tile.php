...
class Olobuild_Twitterfeed_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        <style>.<?php echo $tw_uid; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $tw_uid is internally generated; hover radius CSS built by Olobuild_Tile_Utils::radius_force_css() (absint-forced px values) ?>:hover{border-radius:<?php echo $radius_hover_css; ?> !important}</style>
        <?php endif; ?>
        <div class="olo-twitterfeed <?php echo esc_attr( $tw_uid ); ?>" style="<?php echo esc_attr( $wrapper_style ); ?>">
            <?php if ( empty( $url ) ) : ?>
                <div style="padding:40px 20px;text-align:center;color:var(--olo-color-text-muted, #9CA3AF);background:<?php echo esc_attr( $theme === 'dark' ? '#15202b' : 'var(--olo-color-muted, #F3F4F6)' ); ?>;border-radius:<?php echo esc_attr( $radius ); ?>;width:100%;max-width:550px;">
                    <?php echo esc_html( function_exists( 'olobuild_t' ) ? olobuild_t( 'Inserisci URL profilo X/Twitter' ) : 'Inserisci URL profilo X/Twitter' ); ?>
                </div>
            <?php elseif ( $embed_type === 'tweet' ) : ?>
                ...
                <?php else : ?>
                    <div style="padding:40px 20px;text-align:center;color:var(--olo-color-text-muted, #9CA3AF);">
                        <?php echo esc_html( function_exists( 'olobuild_t' ) ? olobuild_t( 'Impossibile caricare il tweet' ) : 'Impossibile caricare il tweet' ); ?>
                    </div>
                <?php endif; ?>
        ...
        if ( $border_css || $border_hover_css || $border_effect_css ) {
            echo '<style>';
            if ( $border_css ) echo ".{$tw_uid}{{$border_css}}"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base::build_border_css() from sanitized settings; $tw_uid is internally generated
