...
class Olo_MediaCTA_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings, $style = [] ) {
        ...
        ob_start();
        // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above: colours via the safe_color_css() whitelist, overlay via hex_rgb() (digit triplet) with floatval()+min()/max() alphas, paddings/radius integer-forced (pad_css/build_border_radius_css), media background via Olo_CSS_Builder/bg_media_parts or esc_url(), font stacks and ternary branches are fixed literals, KIT declarations via internal helpers; $uid is internally generated.
        ?>
        <style>
        ...
        <section class="olo-mediacta <?php echo esc_attr( $uid ); ?>">
            <div class="omc-media"><?php if ( $mb['has'] && $mb['markup'] !== '' ) { echo $mb['markup']; } // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- media markup generated by Olo_CSS_Builder::get_bg_html_markup() (escapes internally) ?><?php if ( ! $mb['has'] && $img === '' && ! empty( $s['media_label'] ) ) : ?><span class="omc-medialabel"><?php echo esc_html( $s['media_label'] ); ?></span><?php endif; ?></div>
