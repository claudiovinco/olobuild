...
class Olobuild_Testimonial_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        $av_radius   = $is_square ? ( Olobuild_Tile_Utils::radius_int( $s['avatar_radius'] ) . 'px' ) : '50%';
        $tile_radius = Olobuild_Tile_Utils::border_radius( $s['border_radius'] ?? 0 );
        $tile_radius_hover_css = Olobuild_Tile_Utils::radius_force_css( $s['border_radius_hover'] ?? null );
        ...
        if ( $border_css || $border_hover_css || $border_effect_css || $card_border_hover_css ) {
            ...
            echo $border_hover_css . $border_effect_css . $card_border_hover_css . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base border helpers from sanitized border settings
        }
    ...
    private function render_editorial( $uid, $s, $star_color ) {
        ...
        <style>.<?php echo $uid; ?> .olo-test-ed__q em{font-style:italic;color:<?php echo esc_attr( $accent ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $uid is internally generated; color escaped inline ?>;}</style>
        <?php
    }
    ...
    private function render_single( $uid, $s, $star_svg, $is_bottom, $position ) {
        ...
        <div class="olo-testimonial <?php echo esc_attr( $uid ); ?> olo-test-preset-<?php echo esc_attr( sanitize_key( $s['preset'] ?? 'custom' ) ); ?>">
            <div class="olo-test-card">
                <?php echo $this->render_card_inner( $s, $rating, $star_svg, $quote, $author_name, $author_role, $is_bottom, $position ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML assembled by render_card_inner() from values escaped above (esc_html/esc_url/esc_attr) ?>
            </div>
        </div>
        <?php
    }
    ...
    private function render_carousel( $uid, $s, $star_svg, $is_bottom ) {
        ...
        // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below only echoes the internally generated $uid.
        ?>
        <style>
        ...
        <div class="olo-testimonial <?php echo esc_attr( $uid ); ?> olo-test-preset-<?php echo esc_attr( sanitize_key( $s['preset'] ?? 'custom' ) ); ?>"
             uk-slider="autoplay: <?php echo $autoplay_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- fixed 'true'/'false' literal from boolean ternary above ?>; autoplay-interval: <?php echo (int) $interval; ?>">
            <ul class="uk-slider-items uk-child-width-1-<?php echo (int) $slides; ?>@m uk-child-width-1-1">
                ...
                <li>
                    <div class="olo-test-card">
                        ...
                        echo $this->render_card_inner( $item_s, $rating, $star_svg, $quote, $author_name, $author_role, $is_bottom, $s['author_position'] ?? 'bottom-left' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML assembled by render_card_inner() from values escaped above (esc_html/esc_url/esc_attr)
                        ?>
                    </div>
                </li>
                <?php endforeach; ?>
    ...
    private function render_grid( $uid, $s, $star_svg, $is_bottom ) {
        ...
        // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below only echoes the internally generated $uid and the absint()-clamped column count.
        ?>
        <style>
        ...
        if ( ! $is_bottom ) : ?>
            <div class="olo-test-layout">
                <div class="olo-test-author">
                    <?php echo $this->render_author( $s, $author_name, $author_role ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML built by render_author() with esc_url/esc_attr and esc_html()-escaped name/role ?>
                </div>
                <div class="olo-test-quote">
                    <?php echo $this->render_quote( $rating, $star_svg, $quote, $s ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML built by render_quote() from the esc_html()-escaped quote, internal star SVG and tfx_attrs() escaped fragments ?>
                </div>
            </div>
        <?php else : ?>
            <?php echo $this->render_quote( $rating, $star_svg, $quote, $s ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML built by render_quote() from the esc_html()-escaped quote, internal star SVG and tfx_attrs() escaped fragments ?>
