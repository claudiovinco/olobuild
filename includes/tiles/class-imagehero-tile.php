 ...
 */
class Olobuild_ImageHero_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings, $style = [] ) {
        ...
        ob_start();
        ?>
        <?php // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above: safe_color_css() whitelist for every colour, intval()/absint()/floatval() clamps for sizes and overlay alphas, hex_rgb()/clamp01() for the gradient, preg_match() whitelists for aspect-ratio, in_array() whitelists and fixed ternaries for positions/alignments, resolve_font_family() whitelist for fonts, esc_url()/esc_attr() for image URL and label, Olobuild_CSS_Builder/build_border_*_css()/build_shadow_decl() helpers; $uid is internally generated. ?>
        <style>
        ...
        <section class="olo-imagehero <?php echo esc_attr( $uid ); ?>">
            <div class="oih-media"><?php if ( $mb['has'] && $mb['markup'] !== '' ) { echo $mb['markup']; } // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- media markup generated by Olobuild_CSS_Builder::get_bg_html_markup() which escapes its own attributes ?></div>
                ...
                <div class="oih-c">
                    ...
                    <h1 class="oih-h"><?php
                        ...
                        if ( ! empty( $s['accent_text'] ) ) {
                            echo $sep . '<span class="oih-acc">' . esc_html( $s['accent_text'] ) . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $sep is a fixed '<br>'/' ' ternary literal, span markup is static, text escaped via esc_html()
                        }
                        if ( ! empty( $s['headline_tail'] ) ) {
                            echo $sep . esc_html( $s['headline_tail'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $sep is a fixed '<br>'/' ' ternary literal, text escaped via esc_html()
                        }
                    ?></h1>
        ...
        if ( $border_hover_css || $border_effect_css ) {
            echo '<style>' . $border_hover_css . $border_effect_css . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base::build_border_hover_css()/build_border_effect_css() from sanitized border settings
        }
        return ob_get_clean();
    }
