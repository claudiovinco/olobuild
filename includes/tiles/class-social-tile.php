...
class Olo_Social_Tile extends Olo_Tile_Base {
    ...
    public function get_controls() {
        return [
            [ 'key' => 'links', 'type' => 'content-items', 'label' => 'Piattaforme' ],
            ...
            [ 'key' => 'style',     'type' => 'select', 'label' => 'Style', 'options' => [ 'filled' => 'Filled', 'outline' => 'Outline', 'minimal' => 'Minimal' ] ],
            [ 'key' => 'use_brand_colors', 'type' => 'toggle', 'label' => 'Brand Colors' ],
            [ 'key' => 'icon_color', 'type' => 'color', 'label' => 'Custom Color' ],
            [ 'key' => 'show_labels', 'type' => 'toggle', 'label' => 'Show Labels' ],
            [ 'key' => 'hover_effect', 'type' => 'select', 'label' => 'Hover Effect', 'options' => [ 'none' => 'None', 'lift' => 'Lift', 'grow' => 'Grow', 'glow' => 'Glow' ] ],
        ];
    }
    ...
    public function render( $settings ) {
        ...
        $links        = self::parse_links( $s['links'] );
        $size         = absint( $s['size'] );
        $gap          = absint( $s['gap'] );
        ...
        <div id="<?php echo esc_attr( $uid ); ?>" class="olo-social uk-flex uk-flex-wrap <?php echo esc_attr( $align_class ); ?>" style="gap: <?php echo $gap; ?>px; padding: 16px;">
            <?php foreach ( $links as $link ) :
                ...
                if ( $style === 'filled' ) {
                    ...
                    $link_style = sprintf(
                        'width:%dpx;height:%dpx;border:2px solid %s;color:%s;border-radius:50%%;display:inline-flex;align-items:center;justify-content:center;background:transparent;font-size:%dpx;line-height:1;',
                        $size, $size, $this->safe_color_css( $color ), $this->safe_color_css( $color ), intval( $size * 0.5 )
                    );
                } else {
                    $link_style = sprintf(
                        'color:%s;display:inline-flex;align-items:center;justify-content:center;font-size:%dpx;line-height:1;',
                        $this->safe_color_css( $color ), intval( $size * 0.5 )
                    );
                }
            ?>
                <div style="display:flex;flex-direction:column;align-items:center;<?php echo $show_labels ? 'gap:4px;' : ''; ?>">
                    <a href="<?php echo esc_url( $link['url'] ); ?>" target="_blank" rel="noopener noreferrer" class="olo-social-link" style="<?php echo $link_style; ?>" title="<?php echo esc_attr( ucfirst( $link['platform'] ) ); ?>">
                        <?php echo $icon; ?>
                    </a>
                    <?php if ( $show_labels ) : ?>
                        <span style="font-size:<?php echo max( 10, intval( $size * 0.35 ) ); ?>px;color:var(--olo-color-text-muted, #9CA3AF);text-transform:capitalize;"><?php echo esc_html( $link['platform'] ); ?></span>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>
        </div>
    ...
    private static function parse_links( $raw ) {
        // Nuovo formato: array di oggetti { platform, url }
        if ( is_array( $raw ) ) {
            $links = [];
            foreach ( $raw as $item ) {
                if ( is_array( $item ) ) {
        ...
        // Backward compat: vecchio formato testo "platform|url"
        if ( is_string( $raw ) ) {
            $links = [];
