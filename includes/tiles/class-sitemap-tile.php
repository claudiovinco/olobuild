...
class Olo_Sitemap_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        if ( ! empty( $s['show_pages'] ) ) {
            ...
            if ( $pages ) {
                ...
                $sections[] = [ 'heading' => olo_t( 'Pagine' ), 'type' => 'page', 'items' => $items ];
            }
        }
        ...
        if ( ! empty( $s['show_posts'] ) ) {
            ...
            if ( $posts ) {
                ...
                $sections[] = [ 'heading' => olo_t( 'Articoli' ), 'type' => 'post', 'items' => $items ];
            }
        }
        ...
        if ( ! empty( $s['show_categories'] ) ) {
            ...
            if ( $cats ) {
                ...
                if ( $items ) $sections[] = [ 'heading' => olo_t( 'Categorie' ), 'type' => 'cat', 'items' => $items ];
            }
        }
        ...
        if ( ! empty( $s['show_tags'] ) ) {
            ...
            if ( $tags ) {
                ...
                if ( $items ) $sections[] = [ 'heading' => olo_t( 'Tag' ), 'type' => 'tag', 'items' => $items ];
            }
        }
        ...
        if ( ! empty( $s['show_authors'] ) ) {
            ...
            if ( $authors ) {
                ...
                if ( $items ) $sections[] = [ 'heading' => olo_t( 'Autori' ), 'type' => 'author', 'items' => $items ];
            }
        }
        ...
        if ( ! empty( $s['show_archives'] ) ) {
            ...
            if ( $archives_obj ) {
                ...
                if ( $items ) $sections[] = [ 'heading' => olo_t( 'Archivi' ), 'type' => 'archive', 'items' => $items ];
            }
        }
        ...
        if ( empty( $sections ) ) {
            return '<div class="olo-sitemap" style="text-align:center;padding:20px;color:#9CA3AF;">' . olo_t( 'Nessun contenuto trovato per la sitemap.' ) . '</div>';
        }
        ...
        ?>
        <style>
            .<?php echo $uid; ?> { position:relative; gap:<?php echo $gap; ?>px; }
            <?php // Default layout = columns ?>
            .<?php echo $uid; ?>.olo-sm-mode-columns,
            .<?php echo $uid; ?>.olo-sm-mode-cards { display:grid; grid-template-columns:repeat(<?php echo $columns; ?>, 1fr); }
            .<?php echo $uid; ?>.olo-sm-mode-compact { display:grid; grid-template-columns:repeat(<?php echo $columns; ?>, 1fr); }
            ...
            .<?php echo $uid; ?> ul { list-style:none; padding:0; margin:0; line-height:1.6; display:flex; flex-direction:column; gap:<?php echo $item_gap; ?>px; }
        ...
        <?php if ( $extra ) echo '<style>' . $extra . '</style>'; ?>
        ...
        <div class="olo-sitemap olo-sm-preset-<?php echo esc_attr( $preset_id ); ?> olo-sm-mode-<?php echo esc_attr( $layout_mode ); ?> <?php echo esc_attr( $uid ); ?>" style="<?php echo esc_attr( $wrap_styles ); ?>">
            ...
            <div class="olo-sm-section">
                <<?php echo $title_tag; ?> class="olo-sm-title">
                    <?php if ( $show_icons && $section['type'] ) : ?>
                        <span class="olo-sm-icon"><?php echo $this->svg_icon( $section['type'] ); ?></span>
                    <?php endif; ?>
                    <?php echo esc_html( $section['heading'] ); ?>
                    <?php if ( $show_counter ) : ?>
                        <span class="olo-sm-counter"><?php echo count( $section['items'] ); ?></span>
                    <?php endif; ?>
                </<?php echo $title_tag; ?>>
                        ...
                        <li data-depth="<?php echo esc_attr( $depth ); ?>" data-search="<?php echo esc_attr( strtolower( $item['title'] ) ); ?>">
                            ...
                            <a href="<?php echo esc_url( $item['url'] ); ?>">
                                <?php if ( $show_icons && ! empty( $item['icon'] ) && $layout_mode !== 'tree' ) : ?>
                                    <span class="olo-sm-icon"><?php echo $this->svg_icon( $item['icon'] ); ?></span>
                                <?php endif; ?>
    ...
    private function render_cloud( $items, $accent, $link ) {
        ...
        foreach ( $items as $it ) {
            ...
            echo '<a href="' . esc_url( $it['url'] ) . '" style="font-size:' . $size . 'em;font-weight:' . ( $size > 1.3 ? 700 : ( $size > 1 ? 600 : 400 ) ) . ';color:' . esc_attr( $link ) . ';">' . esc_html( $it['title'] ) . '</a>';
            echo '</li>';
        }
        echo '</ul>';
    }
