...
class Olobuild_Gallery_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        <div class="<?php echo esc_attr( $uid ); ?>-filter" id="<?php echo esc_attr( $uid ); ?>-filter">
            <button class="active" data-filter="*"><?php echo esc_html( olobuild_t( 'Tutti' ) ); ?></button>
        ...
        <div class="<?php echo esc_attr( $uid ); ?> olo-gallery-preset-<?php echo esc_attr( sanitize_key( $s['preset'] ?? 'custom' ) ); ?>" id="<?php echo esc_attr( $uid ); ?>" uk-lightbox="animation: <?php echo $lb_anim; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped via esc_attr() at assignment above ?>">
            ...
            foreach ( $images as $img ) :
                ...
                if ( '' === trim( (string) $alt ) ) {
                    $link_label = ( ! empty( $caption ) )
                        ? $caption
                        : sprintf( olobuild_t( 'Apri immagine %d' ), $i );
                }
            ...
            ?>
                ...
                <a class="olo-gal-item" href="<?php echo esc_url( $url ); ?>"<?php echo $caption_attr . $cat_attr . $aria_label_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- attribute strings assembled above from esc_attr()'d values and integer counters only ?>>
                    <?php echo Olobuild_Tile_Utils::img_srcset( $att_id, $url, $alt ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- img_srcset() returns an <img> tag escaped internally (esc_url/esc_attr/intval). ?>
