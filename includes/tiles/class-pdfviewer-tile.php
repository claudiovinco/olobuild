...
class Olobuild_PdfViewer_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        if ( empty( $pdf_url ) ) {
            ...
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- lettura read-only del flag di routing iframe builder; nessuna modifica di stato; valore sanitizzato e usato solo come booleano
            if ( ! empty( sanitize_text_field( wp_unslash( $_GET['olo_builder_iframe'] ?? '' ) ) ) ) {
        ...
        <div class="<?php echo esc_attr( $uid ); ?>"
             data-olo-pdfviewer='<?php echo wp_json_encode( $config ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- JSON attribute built by wp_json_encode() from esc_url()'d URL, whitelisted enums, sanitize_text_field()'d zoom and (bool)/(int) casts ?>'
