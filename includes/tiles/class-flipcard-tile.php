...
class Olobuild_FlipCard_Tile extends Olobuild_Tile_Base {
    ...
    protected $defaults = [
        'preset' => 'custom',
        // Sfondo unificato per faccia (pannello universale) — precede i campi legacy.
    ...
    public function render( $settings ) {
        ...
        $radius   = Olobuild_Tile_Utils::border_radius( $s['card_border_radius'] ?? 0 );
        $radius_hover_css = Olobuild_Tile_Utils::radius_force_css( $s['card_border_radius_hover'] ?? null );
        ...
        $padding = Olobuild_Tile_Utils::spacing_css( $s['tile_padding'] ?? $s['card_padding'] ?? 24, 24 );
        $halfH    = round( $height / 2 );

        // Sfondo unificato per faccia: pannelli front_media / back_media
        // (immagine/video/gradiente/…) con precedenza sui campi legacy
        // front_image/front_video e back_image/back_video (tenuti come fallback).
        $fm = $this->bg_media_parts( $s['front_media'] ?? null, $uid . '-f' );
        $bm = $this->bg_media_parts( $s['back_media'] ?? null, $uid . '-b' );
        ...
        ob_start();
        ?>
        <?php // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above: intval()/absint() for all sizes and durations, safe_color_css() whitelist for every colour, in_array() whitelists for direction/trigger/alignments, esc_attr() for easing, Olobuild_Tile_Utils helpers (border_radius/radius_force_css/shadow_value/spacing_css) and fixed transform strings from internal switch maps. ?>
        <style>
            .<?php echo $uid; ?> {
            ...
            <?php if ( $fm['has'] && $fm['css'] !== '' ) : ?>
            .<?php echo $uid; ?> .olo-fc-front .olo-fc-bg { <?php echo $fm['css']; ?> }
            <?php endif; ?>
            <?php if ( $bm['has'] && $bm['css'] !== '' ) : ?>
            .<?php echo $uid; ?> .olo-fc-back .olo-fc-bg { <?php echo $bm['css']; ?> }
            <?php endif; ?>
        ...
        <div class="olo-fc <?php echo esc_attr( $uid ); ?> olo-fc-preset-<?php echo esc_attr( sanitize_key( $s['preset'] ?? 'custom' ) ); ?>" data-trigger="<?php echo esc_attr( $trigger ); ?>">
            <div class="olo-fc-inner">
                <div class="olo-fc-front">
                    <?php echo $this->render_face( $s, 'front', $fm ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- markup generated by render_face() which escapes every value internally (esc_html/esc_attr/esc_url/safe_color_css/Olobuild_Tile_Utils::img_srcset) and by Olobuild_CSS_Builder::get_bg_html_markup() which escapes its own output ?>
                </div>
                <div class="olo-fc-back">
                    <?php echo $this->render_face( $s, 'back', $bm ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- markup generated by render_face() which escapes every value internally (esc_html/esc_attr/esc_url/safe_color_css/Olobuild_Tile_Utils::img_srcset) and by Olobuild_CSS_Builder::get_bg_html_markup() which escapes its own output ?>
                </div>
            </div>
        </div>
        <script>
        ...
        if ( $border_css || $border_hover_css || $border_effect_css || $card_border_hover_css ) {
            ...
            echo $border_hover_css . $border_effect_css; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base::build_border_hover_css()/build_border_effect_css() shared helpers
            echo $card_border_hover_css . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base::build_border_hover_css() shared helper
        }
     ...
     * @param array|null $media Parti sfondo unificato (front_media/back_media) da bg_media_parts().
     */
    private function render_face( $s, $side, $media = null ) {
        $prefix = $side . '_';
        $html   = '';

        // Sfondo unificato (pannello *_media): precede i campi legacy immagine/video/overlay.
        // Il CSS (gradiente/colore/immagine) è già nel blocco <style> scoped su .olo-fc-bg;
        // qui esce solo il markup (video/gallery). Il div .olo-fc-bg fa da wrapper posizionato.
        $has_media = is_array( $media ) && ! empty( $media['has'] );
        if ( $has_media ) {
            $html .= '<div class="olo-fc-bg">' . $media['markup'] . '</div>'; // markup escaped internally by Olobuild_CSS_Builder::get_bg_html_markup()
            return $html . $this->render_face_content( $s, $side );
        }
        ...
        if ( ! empty( $s[ $prefix . 'image' ] ) ) {
            ...
            if ( $img_pad > 0 || $img_rad > 0 ) {
                ...
                $html .= Olobuild_Tile_Utils::img_srcset( absint( $s[ $prefix . 'image_id' ] ?? 0 ), $s[ $prefix . 'image' ], ucfirst( $side ) . ' side', 'olo-fc-bg', 'full', 'style="' . esc_attr( $img_obj_fit ) . '"' );
            }
        }
        ...
        // Content
        $html .= $this->render_face_content( $s, $side );

        return $html;
    }

    /**
     * Render the content column (icon/title/description/CTA) wrapped in .olo-fc-content.
     * Condiviso fra il percorso sfondo unificato (*_media) e il fallback legacy.
     */
    private function render_face_content( $s, $side ) {
        $prefix = $side . '_';
