...
class Olobuild_Code_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        <div class="olo-code" id="<?php echo esc_attr( $uid ); ?>" style="border-radius:8px;overflow:hidden;background:<?php echo esc_attr( $t['bg'] ); ?>;border:1px solid <?php echo esc_attr( $t['line'] ); ?>;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 16px;background:<?php echo esc_attr( $t['header'] ); ?>;border-bottom:1px solid <?php echo esc_attr( $t['line'] ); ?>;">
                ...
                <button type="button" class="olo-code-copy" onclick="(function(btn){var el=btn.closest('.olo-code');if(el){var c=el.querySelector('code');if(c){var t=c.textContent;if(navigator.clipboard){navigator.clipboard.writeText(t)}btn.textContent='<?php echo esc_js( olobuild_t( 'Copiato!' ) ); ?>';setTimeout(function(){btn.textContent='<?php echo esc_js( olobuild_t( 'Copy' ) ); ?>'},2000)}}})(this)" style="font-size:12px;font-family:monospace;color:<?php echo esc_attr( $t['text'] ); ?>;opacity:0.7;background:transparent;border:1px solid <?php echo esc_attr( $t['line'] ); ?>;border-radius:4px;padding:2px 8px;cursor:pointer;"><?php echo esc_html( olobuild_t( 'Copy' ) ); ?></button>
                <?php endif; ?>
            </div>
            <?php endif; ?>
            <pre class="<?php echo esc_attr( $lang_class ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $max_height_style from intval() + fixed literals, $white_space a fixed 'pre'/'pre-wrap' literal, $code esc_html()'d above (line-number markup is assembled from esc_html()/esc_attr()'d parts only) ?>" style="margin:0;padding:1em;overflow-x:auto;background:transparent;<?php echo $max_height_style; ?>"><code class="<?php echo esc_attr( $lang_class ); ?>" style="font-family:monospace;font-size:<?php echo (int) $font_size; ?>px;color:<?php echo esc_attr( $t['text'] ); ?>;line-height:1.6;white-space:<?php echo $white_space; ?>;"><?php echo $code; ?></code></pre>
        </div>
        <?php
                // Border system
