...
class Olo_ServiceDirections_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        $sd_uid = 'olo-sdir-' . wp_unique_id();
        if ( $radius_hover_css !== '' ) $wrap_styles[] = 'transition:border-radius 400ms cubic-bezier(.4,0,.2,1)';

        ob_start();
        ?>
        <?php if ( $radius_hover_css !== '' ) : ?>
        <style>.<?php echo $sd_uid; ?>:hover{border-radius:<?php echo $radius_hover_css; ?> !important}</style>
        <?php endif; ?>
        <div class="<?php echo $sd_uid; ?>" style="<?php echo implode( ';', $wrap_styles ); ?>">
