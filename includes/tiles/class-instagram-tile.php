...
class Olobuild_Instagram_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        <style>.<?php echo $ig_uid; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $ig_uid is internally generated; radius built by Olobuild_Tile_Utils::radius_force_css() from absint() values ?>:hover{border-radius:<?php echo $radius_hover_css; ?> !important}</style>
        <?php endif; ?>
        <div class="olo-instagram <?php echo esc_attr( $ig_uid ); ?>" style="<?php echo esc_attr( $wrapper_style ); ?>">
            <?php if ( empty( $url ) ) : ?>
                <div style="padding:40px 20px;text-align:center;color:var(--olo-color-text-muted, #9CA3AF);background:var(--olo-color-muted, #F3F4F6);border-radius:<?php echo esc_attr( $radius ); ?>;">
                    <?php echo esc_html( function_exists( 'olobuild_t' ) ? olobuild_t( 'Inserisci un URL Instagram' ) : 'Inserisci un URL Instagram' ); ?>
                </div>
            <?php elseif ( $embed_type === 'profile' ) : ?>
                ...
                <?php else : ?>
                    <div style="padding:40px 20px;text-align:center;color:var(--olo-color-text-muted, #9CA3AF);">
                        <?php echo esc_html( function_exists( 'olobuild_t' ) ? olobuild_t( 'URL profilo Instagram non valido' ) : 'URL profilo Instagram non valido' ); ?>
                    </div>
                <?php endif; ?>
            <?php else : ?>
                <?php
                ...
                ?>
                    ...
                    >
                        <a href="<?php echo esc_url( $url ); ?>" target="_blank" rel="noopener noreferrer">
                            <?php echo esc_html( function_exists( 'olobuild_t' ) ? olobuild_t( 'Visualizza su Instagram' ) : 'Visualizza su Instagram' ); ?>
                        </a>
                    </blockquote>
                <?php endif; ?>
        ...
        if ( $border_css || $border_hover_css || $border_effect_css ) {
            echo '<style>';
            if ( $border_css ) echo ".{$ig_uid}{{$border_css}}"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base::build_border_css() from sanitized settings; $ig_uid is internally generated
