        ...
        <div class="olo-hoverlist <?php echo esc_attr( $uid ); ?>" style="position:relative;border-top:1px solid <?php echo esc_attr( $line ); ?>;">
            ...
            ?>
                <<?php echo $tag . $attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $tag is a fixed 'a'/'div' literal from the ternary above; $attrs is empty or built with esc_url() ?> class="olo-hoverlist__row" data-color="<?php echo esc_attr( $color ); ?>" data-name="<?php echo esc_attr( $name ); ?>" data-image="<?php echo esc_url( $pimg ); ?>" style="<?php echo esc_attr( $row_base ); ?>">
                    ...
                    <span class="olo-hoverlist__nm" style="font-family:<?php echo esc_attr( $nfam ); ?>;font-size:<?php echo (int) $name_sz; ?>px;color:<?php echo esc_attr( $name_c ); ?>;line-height:1.1;" data-olo-editable="<?php echo 'items.' . intval( $idx ) . '.name'; ?>"><?php echo esc_html( $name ); ?></span>
                    <?php if ( $sub !== '' ) : ?>
