...
class Olo_Osmmap_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        <style>#<?php echo esc_attr( $map_id ); ?>{transition:border-radius 400ms cubic-bezier(.4,0,.2,1)}#<?php echo esc_attr( $map_id ); ?>:hover{border-radius:<?php echo $radius_hover_css; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- absint-built radius from Olo_Tile_Utils::radius_force_css() ?> !important}</style>
        <?php endif; ?>

        <div id="<?php echo esc_attr( $map_id ); ?>" class="olo-osmmap" style="height:<?php echo (int) $height; ?>px; border-radius:<?php echo $radius; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- absint-built radius from Olo_Tile_Utils::border_radius() ?>; overflow:hidden;"></div>
        ...
        $border_hover_css  = $this->build_border_hover_css( ".{$map_id}", $s['border'] ?? [], $s['border_hover'] ?? [], intval( $s['border_hover_duration'] ?? 300 ) );
        $border_effect_css = $this->build_border_effect_css( ".{$map_id}", $s['border'] ?? [], $s );
        if ( $border_css || $border_hover_css || $border_effect_css ) {
            echo '<style>';
            if ( $border_css ) echo ".{$map_id}{{$border_css}}"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olo_Tile_Base::build_border_css() from sanitized settings; $map_id is internally generated
