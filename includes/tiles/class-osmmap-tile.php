...
class Olobuild_Osmmap_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        wp_enqueue_style( 'leaflet', OLOBUILD_URL . 'assets/vendor/leaflet/leaflet.css', [], OLOBUILD_VERSION );
        wp_enqueue_script( 'leaflet', OLOBUILD_URL . 'assets/vendor/leaflet/leaflet.js', [], OLOBUILD_VERSION, true );

        ob_start();
        ?>
        <?php if ( $radius_hover_css !== '' ) : ?>
        <style>#<?php echo esc_attr( $map_id ); ?>{transition:border-radius 400ms cubic-bezier(.4,0,.2,1)}#<?php echo esc_attr( $map_id ); ?>:hover{border-radius:<?php echo $radius_hover_css; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- absint-built radius from Olobuild_Tile_Utils::radius_force_css() ?> !important}</style>
        <?php endif; ?>
        ...
        if ( $border_css || $border_hover_css || $border_effect_css ) {
            echo '<style>';
            if ( $border_css ) echo ".{$map_id}{{$border_css}}"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base::build_border_css() from sanitized settings; $map_id is internally generated
