...
class Olobuild_Accordion_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        // Raggio in hover: sulle voci quando sono staccate (gap), sulla lista quando sono unite.
        $rad_h = Olobuild_Tile_Utils::radius_hover( $s, 'border_radius_hover' );
        $gap         = intval( $s['gap'] );
        ...
        <style>
            .<?php echo esc_attr( $uid ); ?> .uk-accordion-title {
                ...
                gap: 12px;
                cursor: pointer;
                transition: background <?php echo $speed; ?>ms ease, color <?php echo $speed; ?>ms ease, box-shadow <?php echo $speed; ?>ms ease<?php if ( $rad_h && $gap > 0 ) echo ', ' . $rad_h['transition']; ?>;
