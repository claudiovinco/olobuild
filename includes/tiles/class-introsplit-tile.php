...
class Olo_IntroSplit_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings, $style = [] ) {
        ...
        <div class="ois-media"<?php echo $media_sty; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- style attribute built above from esc_attr()'d bg CSS or an esc_url()'d image URL only ?>>
            <?php if ( $mb['has'] && $mb['markup'] !== '' ) { echo $mb['markup']; } // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- media markup generated by Olo_CSS_Builder::get_bg_html_markup(), which escapes its own output ?>
        ...
        <div class="olo-introsplit <?php echo esc_attr( $uid ); ?>">
            <?php if ( $left ) : ?><div class="ois-mediawrap"><?php echo $media_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- fragment buffered above with every dynamic value escaped at build time (esc_html/esc_url/esc_attr, Olo_CSS_Builder markup) ?></div><?php endif; ?>
