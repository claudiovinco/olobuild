...
class Olobuild_Mobilebar_Tile extends Olobuild_Tile_Base {
    ...
    protected $defaults = [
        ...
        'hamburger_size'     => '28',
        'hamburger_color'    => 'var(--olo-color-light, #ffffff)',
        'menu_id'            => '',
        'panel_bg'           => 'var(--olo-color-light, #ffffff)',
        'panel_text_color'   => 'var(--olo-color-dark, #222222)',
        'panel_active_color' => '',
        'panel_font_size'    => '17',
        ...
        'panel_chevron_color'=> '',
        'search_enabled'     => true,
        'search_icon_color'  => 'var(--olo-color-light, #ffffff)',
    ...
    public function render( $settings ) {
        ...
        if ( is_array( $p_pad_raw ) ) {
            ...
            $p_pad_css = Olobuild_Tile_Utils::spacing_css( $p_pad_raw, 16 );
        } else {
    ...
    private function render_css( $uid, $bp, $bar_h, $bar_bg, $bar_pad, $bar_shadow, $ham_color, $ham_size, $p_bg, $p_color, $p_active, $p_fs, $p_pad, $p_sep, $p_chev, $search_ic, $p_pad_css = '' ) {
        ...
        .<?php echo $uid; ?> .olo-mb-bar {
            ...
            padding: 0 <?php echo (int) $bar_pad; ?>px;
        ...
        .<?php echo $uid; ?> .olo-mb-hamburger {
            width: <?php echo (int) $ham_size; ?>px;
            height: <?php echo (int) $ham_size; ?>px;
            position: relative;
        ...
        .<?php echo $uid; ?>.olo-mb-open .olo-mb-dropdown {
            max-height: calc(100vh - <?php echo (int) $bar_h; ?>px);
            overflow-y: auto;
        }
        ...
        .<?php echo $uid; ?> .olo-mb-nav > ul > li > a {
            ...
            text-decoration: none;
            font-size: <?php echo (int) $p_fs; ?>px;
            font-weight: 500;
            line-height: 1.4;
        }
        ...
        .<?php echo $uid; ?> .olo-mb-search-form {
            display: flex;
            padding: 10px <?php echo (int) $bar_pad; ?>px;
            gap: 8px;
        }
    ...
    private function render_html( $uid, $logo_img, $logo_w, $logo_link, $ham_style, $search_on, $search_ph, $menu_id, $bar_h, $root_pad_sides = [ 'top' => 0, 'right' => 0, 'bottom' => 0, 'left' => 0 ] ) {
        ...
        <div class="olo-mobilebar <?php echo esc_attr( $uid ); ?>" data-uid="<?php echo esc_attr( $uid ); ?>"<?php if ( array_sum( $root_pad_sides ) > 0 ) : ?> style="padding:<?php echo esc_attr( Olobuild_Tile_Utils::sides_css( $root_pad_sides ) ); ?>"<?php endif; ?>>
            ...
            <div class="olo-mb-bar">
                <a class="olo-mb-logo" href="<?php echo $logo_link; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped via esc_url() at assignment in render() ?>">
                    <?php if ( $logo_img ) : ?>
                ...
                <?php if ( $search_on ) : ?>
                <button class="olo-mb-icon-btn olo-mb-search-btn" aria-label="<?php echo esc_attr( olobuild_t( 'Cerca' ) ); ?>" type="button">
                    <?php echo $search_svg; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG markup defined above in this method ?>
                </button>
                <?php endif; ?>

                <button class="olo-mb-icon-btn olo-mb-hamburger olo-mb-ham-<?php echo esc_attr( $ham_style ); ?>" aria-label="<?php echo esc_attr( olobuild_t( 'Menu' ) ); ?>" aria-expanded="false" type="button">
            ...
            <div class="olo-mb-search-panel">
                <form class="olo-mb-search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>" method="get" role="search">
                    <input type="search" name="s" placeholder="<?php echo esc_attr( $search_ph ); ?>" autocomplete="off">
                    <button type="submit" aria-label="<?php echo esc_attr( olobuild_t( 'Cerca' ) ); ?>"><?php echo $search_svg; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG markup defined above in this method ?></button>
                </form>
            </div>
            <?php endif; ?>
            ...
            <div class="olo-mb-dropdown">
                <nav class="olo-mb-nav" aria-label="<?php echo esc_attr( olobuild_t( 'Mobile navigation' ) ); ?>">
    ...
    private function render_menu_items( $menu_id, $current_url, $chevron_svg ) {
        ...
        foreach ( $tree as $item ) {
            ...
            echo '<li' . $cls . '>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $cls is built above from literals and esc_attr()-escaped class names
            ...
            if ( ! empty( $subs ) ) {
                ...
                foreach ( $subs as $sub ) {
                    ...
                    echo '<li' . $sc . '>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $sc is built above from literals and esc_attr()-escaped class names
                    if ( ! empty( $sub_subs ) ) {
                        echo '<div class="olo-mb-item">';
