...
class Olo_Mobilebar_Tile extends Olo_Tile_Base {
    ...
    private function render_css( $uid, $bp, $bar_h, $bar_bg, $bar_pad, $bar_shadow, $ham_color, $ham_size, $p_bg, $p_color, $p_active, $p_fs, $p_pad, $p_sep, $p_chev, $search_ic, $p_pad_css = '' ) {
        ...
        // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized in render() (safe_color_css(), intval()/max()/round() numerics, Olo_Tile_Utils::spacing_css()) and the internally generated $uid.
        ?>
        <style>
        ...
        <div class="olo-mobilebar <?php echo esc_attr( $uid ); ?>" data-uid="<?php echo esc_attr( $uid ); ?>">
            ...
            <div class="olo-mb-bar">
                <a class="olo-mb-logo" href="<?php echo $logo_link; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped via esc_url() at assignment in render() ?>">
                    <?php if ( $logo_img ) : ?>
                        <img src="<?php echo $logo_img; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped via esc_url() at assignment in render() ?>" alt="<?php echo esc_attr( olo_t( 'Logo' ) ); ?>" style="max-width:<?php echo (int) $logo_w; ?>px;">
                    <?php else : ?>
                ...
                <button class="olo-mb-icon-btn olo-mb-hamburger olo-mb-ham-<?php echo esc_attr( $ham_style ); ?>" aria-label="<?php echo esc_attr( olo_t( 'Menu' ) ); ?>" aria-expanded="false" type="button">
            ...
            <div class="olo-mb-search-panel">
                <form class="olo-mb-search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>" method="get" role="search">
                    ...
                    <button type="submit" aria-label="<?php echo esc_attr( olo_t( 'Cerca' ) ); ?>"><?php echo $search_svg; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG markup defined above in this method ?></button>
                </form>
            </div>
            <?php endif; ?>
