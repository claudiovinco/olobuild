...
class Olo_Panel_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        ob_start();
        ?>
        <style><?php echo $css; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by build_scoped_css() (absint()/intval()/in_array() whitelists/esc_attr() on every setting, Olo_Tile_Utils helpers) and the Olo_Tile_Base::build_wow_effects_css() shared helper ?></style>
        <div class="olo-panel olo-pn--preset-<?php echo esc_attr( $preset_id ); ?> <?php echo esc_attr( $uid ); ?> <?php echo esc_attr( $card_class ); ?>">
            <?php if ( $media_type === 'image' && ! empty( $s['image'] ) ) : ?>
                <div class="olo-panel-media uk-card-media-top">
                    ...
                    echo $this->render_hover_wrap( $panel_img, $s['hover_image'] ?? '', $s['hover_video'] ?? '' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $panel_img built by Olo_Tile_Utils::img_srcset() (escapes internally); render_hover_wrap() escapes hover media URLs internally
                    ?>
                </div>
            ...
            <?php if ( ! empty( $s['title'] ) ) : ?>
                <<?php echo tag_escape( $tag ); ?> class="uk-card-title olo-panel-title<?php echo $t_tfx_cls; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- tfx_attrs() fragments are escaped internally by Olo_Text_Effects (sanitize_html_class/esc_attr) ?>"<?php echo $t_tfx_data; ?>><?php
                    if ( ! empty( $s['link_url'] ) ) {
            ...
            <?php if ( ! empty( $s['content'] ) ) : ?>
                <div class="olo-panel-content<?php echo $c_tfx_cls; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- tfx_attrs() fragments are escaped internally by Olo_Text_Effects (sanitize_html_class/esc_attr); content is esc_html()'d (nl2br only adds <br /> tags) ?>"<?php echo $c_tfx_data; ?>><?php echo nl2br( esc_html( $content_plain ) ); ?></div>
            <?php endif; ?>
