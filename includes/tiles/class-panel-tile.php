...
class Olobuild_Panel_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        // Raggio card in hover: stesso elemento del bordo in hover; si tengono la sua transizione e quella della card UIkit.
        $tr_card          = 'box-shadow .1s ease-in-out'; // la transition di .uk-card (UIkit), da conservare
        $tr_bordo         = Olobuild_Tile_Utils::transizione_di( $border_hover_css );
        $radius_hover_css = Olobuild_Tile_Utils::radius_hover_rules( ".{$uid}.olo-panel", $s, 'card_radius_hover', $tr_card . ( $tr_bordo !== '' ? ', ' . $tr_bordo : '' ) );
