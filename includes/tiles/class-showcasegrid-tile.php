 ...
 */
class Olobuild_ShowcaseGrid_Tile extends Olobuild_Tile_Base {
    ...
    protected $defaults = [
        ...
        // Il media e' un background: l'adattamento si traduce in background-size,
        // e 'cover' e' il valore cablato finora → default no-op.
        'object_fit'        => 'cover',
        'radius'            => 20,
    ...
    public function render( $settings, $style = [] ) {
        ...
        // Adattamento → background-size (il media e' un background-image, non un <img>).
        // Tabella di traduzione = whitelist: qui dentro c'e' tutto e solo cio' che il
        // select offre. 'scale-down' non ha equivalente e infatti non e' offerto.
        $fit_map = [ 'cover' => 'cover', 'contain' => 'contain', 'fill' => '100% 100%', 'none' => 'auto' ];
        $bg_size = $fit_map[ (string) ( $s['object_fit'] ?? 'cover' ) ] ?? 'cover';
        ...
        <div class="olo-showcasegrid <?php echo esc_attr( $uid ); ?>">
            ...
            ?>
                <a class="ocg-card" href="<?php echo esc_url( $href ); ?>"<?php echo $card_inline; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- style attr built from intval span and aspect filtered to [0-9./] via preg_replace above ?>>
                    ...
                    <?php if ( $mb['has'] && $mb['markup'] !== '' ) { echo $mb['markup']; } // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- media markup generated by Olobuild_CSS_Builder::get_bg_html_markup() (escapes internally) ?>
        ...
        if ( $border_hover_css || $border_effect_css ) {
            echo '<style>' . $border_hover_css . $border_effect_css . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS built by Olobuild_Tile_Base border helpers from sanitized values
        }
        return ob_get_clean();
    }
