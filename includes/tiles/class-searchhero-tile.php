 ...
 */
class Olo_SearchHero_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings, $style = [] ) {
        ...
        ob_start();
        ?>
        <?php // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above: every colour via the safe_color_css() whitelist (or fixed literal fallback), padding/radius from intval()'d sides, fixed font-stack literals, min-height via intval() clamp, bg via Olo_CSS_Builder::get_bg_inline_css(), shadow via build_shadow_decl() (intval/safe_color_css/fixed map); $uid is internally generated. ?>
        <style>
        ...
        if ( $border_css || $border_hover_css || $border_effect_css ) {
            echo '<style>';
            if ( $border_css ) {
                echo ".{$uid}{{$border_css}}"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olo_Tile_Base::build_border_css() from sanitized settings; $uid is internally generated
            }
