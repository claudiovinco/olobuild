...
class Olobuild_HeroSplit_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings, $style = [] ) {
        ...
        $grid_style = 'display:grid;grid-template-columns:' . ( $panel_visible ? $split : '1fr' ) . ';gap:' . $gap . 'px;align-items:center;';
                ...
                <?php if ( $panel_visible && 'showcase' !== $panel ) : ?>
                    <div class="olo-hsplit__right" style="position:relative;display:flex;flex-direction:column;justify-content:center">
                        <div style="position:relative;aspect-ratio:<?php echo esc_attr( $panel_ar ); ?>">
                            ...
                            <?php if ( 'media' === $panel && ( ! empty( $s['panel_badge_number'] ) || ! empty( $s['panel_badge_label'] ) ) ) : ?>
                            <div style="position:absolute;left:-18px;bottom:24px;z-index:2;background:var(--olo-color-primary, #e1474f);color:var(--olo-color-on-primary, #ffffff);border-radius:16px;padding:18px 22px;box-shadow:0 18px 40px -16px rgba(22,38,61,.5)">
                                <?php if ( ! empty( $s['panel_badge_number'] ) ) : ?><b style="font-family:<?php echo esc_attr( $headline_family ); ?>;font-weight:900;font-size:34px;display:block;line-height:1"><?php echo esc_html( $s['panel_badge_number'] ); ?></b><?php endif; ?>
                                <?php if ( ! empty( $s['panel_badge_label'] ) ) : ?><span style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.06em"><?php echo esc_html( $s['panel_badge_label'] ); ?></span><?php endif; ?>
                            </div>
                            <?php endif; ?>
                        </div>
                        <?php if ( 'audio' === $panel ) : ?>
                        <div style="display:flex;align-items:center;gap:14px;margin-top:16px;background:<?php echo esc_attr( $mixp( 5 ) ); ?>;border:1px solid <?php echo esc_attr( $mixp( 10 ) ); ?>;border-radius:14px;padding:14px 16px">
                            <span aria-hidden="true" style="width:42px;height:42px;border-radius:50%;background:var(--olo-color-primary, #e1474f);color:var(--olo-color-on-primary, #ffffff);display:grid;place-items:center;flex:none"><svg viewBox="0 0 24 24" fill="currentColor" style="width:16px;height:16px"><path d="M8 5v14l11-7z"/></svg></span>
                            <div style="flex:1;min-width:0"><b style="display:block;font-weight:700;font-size:14px"><?php echo esc_html( $s['panel_track_title'] ); ?></b><span style="font-size:12px;opacity:.65"><?php echo esc_html( $s['panel_track_meta'] ); ?></span></div>
                            <div aria-hidden="true" style="display:flex;align-items:center;gap:2px;height:26px;flex:none"><?php foreach ( $wave as $h ) : ?><span style="width:2.5px;background:var(--olo-color-primary, #e1474f);border-radius:2px;opacity:.5;height:<?php echo (int) $h; ?>%"></span><?php endforeach; ?></div>
                        </div>
                        <?php endif; ?>
                    </div>
                <?php elseif ( $panel_visible ) :
