  ...
  mo.observe(document.body, { childList:true, subtree:true });
    ...
    private function render_image_block( $s, $img_class, $link_url, $link_target ) {
        ...
        if ( ! empty( $link_url ) ) {
            ...
            echo '<a href="' . esc_url( $link_url ) . '"' . $target_attr . '>' . $img_html . '</a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $target_attr is a fixed literal from the ternary above; image HTML built by Olo_Tile_Utils::img_srcset()/render_hover_wrap() which escape internally
        } else {
            echo $img_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- image HTML built by Olo_Tile_Utils::img_srcset()/render_hover_wrap() which escape internally
        }
    }
}
