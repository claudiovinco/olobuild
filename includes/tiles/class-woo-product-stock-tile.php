...
class Olobuild_Woo_Product_Stock_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        if ( $stock_status === 'onbackorder' ) {
            $status_text  = olobuild_t( 'Disponibile su ordinazione' );
            $status_color = $low_stock_color;
        } elseif ( ! $is_in_stock ) {
            $status_text  = olobuild_t( 'Non disponibile' );
            $status_color = $out_of_stock_color;
        } elseif ( $manages_stock ) {
            if ( $stock_qty !== null ) {
                if ( $stock_qty <= $low_threshold ) {
                    $status_text  = sprintf( olobuild_t( 'Solo %d rimasti' ), $stock_qty );
                    $status_color = $low_stock_color;
                } else {
                    $status_text  = olobuild_t( 'Disponibile' );
        ...
        ob_start();
        // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above: the internally generated $uid, absint()/min()/max() clamped sizes, in_array() whitelisted weight and the safe_color_css()-validated (or fixed var() fallback) status colour.
        ?>
        <style>
            .<?php echo $uid; ?> {
                display: flex;
