...
class Olobuild_Floatingpanel_Tile extends Olobuild_Tile_Base {
    ...
    public function render_closing( $settings ) {
        ...
        if ( $radius_hover_css === '' && $tr_bordo !== '' ) {
            $radius_hover_css = ".{$uid}-panel{transition:{$tr_insieme}}";
        }
        if ( $border_css || $border_hover_css || $border_effect_css || $radius_hover_css ) {
            ...
            echo $border_hover_css . $border_effect_css . $radius_hover_css . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base::build_border_hover_css()/build_border_effect_css() from sanitized border settings; radius rules from Olobuild_Tile_Utils::radius_hover_rules() (integer px)
        }
        return ob_get_clean();
    }
