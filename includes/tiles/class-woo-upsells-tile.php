...
class Olobuild_Woo_Upsells_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        if ( empty( $upsell_ids ) ) {
            return '<div style="padding:20px;text-align:center;color:var(--olo-color-text-muted, #6B7280);font-size:14px;">'
                 . esc_html( olobuild_t( 'Nessun prodotto suggerito configurato' ) )
                 . '</div>';
        }
        ...
        if ( ! $upsells->have_posts() ) {
            return '<div style="padding:20px;text-align:center;color:var(--olo-color-text-muted, #6B7280);font-size:14px;">'
                 . esc_html( olobuild_t( 'Nessun prodotto suggerito trovato' ) )
                 . '</div>';
        }

        ob_start();
        ?>
        <?php // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above (safe_color_css/absint/internal uid/fixed strings). ?>
        <style>
        ...
        <div class="<?php echo esc_attr( $uid ); ?>">
            ...
            <h2 class="olo-ups-heading"><?php echo esc_html( olobuild_t( $s['title_text'] ) ); ?></h2>
            <?php endif; ?>
                ...
                <div class="olo-ups-card">
                    ...
                    <div class="olo-ups-img">
                        ...
                        <a href="<?php echo esc_url( $permalink ); ?>">
                            <?php echo wp_get_attachment_image( $thumb_id, 'woocommerce_thumbnail' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- wp_get_attachment_image() returns escaped image markup ?>
                        </a>
                        <?php else : ?>
                    ...
                    <div class="olo-ups-body">
                        ...
                        <div class="olo-ups-price"><?php echo $price_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- WC_Product::get_price_html() returns pre-escaped WooCommerce price markup ?></div>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endwhile; ?>
