 ...
 */
class Olobuild_GlowGallery_Tile extends Olobuild_Tile_Base {
    ...
    protected $defaults = [
        ...
        'strip_radius'   => 200,
        // Aspetto / glow
        'bg_color'       => 'var(--olo-color-dark, #241430)',
        'accent'         => '',
        'accent_on'      => 'var(--olo-color-dark, #170c1f)',
        'text_color'     => 'var(--olo-color-light, #f3e9ef)',
        'sub_color'      => 'var(--olo-color-text-faint, #c8b3c6)',
        'eyebrow_color'  => 'var(--olo-color-accent, #e0afca)',
        'media_bg'       => 'var(--olo-color-dark, #33203f)',
        'media_ratio'    => '3/4',
    ...
    public function render( $settings, $style = [] ) {
        ...
        // Proporzione delle tessere: '3/4' era cablato qui dentro. Whitelist "W/H"
        // come in Olobuild_Tile_Utils::image_frame(); niente altro finisce nel CSS.
        $mratio = is_string( $s['media_ratio'] ?? null ) ? trim( $s['media_ratio'] ) : '3/4';
        $mratio = str_replace( ':', '/', $mratio );
        if ( ! preg_match( '/^\d+(?:\.\d+)?(?:\s*\/\s*\d+(?:\.\d+)?)?$/', $mratio ) ) {
            $mratio = '3/4';
        }
