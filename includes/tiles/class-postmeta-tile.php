...
class Olobuild_PostMeta_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        if ( ! empty( $s['show_comments_count'] ) ) {
            ...
            $label = $count . ' ' . olobuild_t( $count === 1 ? 'commento' : 'commenti' );
        ...
        if ( $extra ) {
            echo '<style>' . $extra . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base::build_wow_effects_css() from clamped numerics and whitelisted enums
        }
