...
class Olobuild_Woo_Minicart_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        ob_start();
        ?>
        <?php // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above: colours via the safe_color_css() whitelist, dropdown width via absint() with min()/max() clamp; $uid is internally generated. ?>
        <style>
            .<?php echo $uid; ?> {
                display: inline-flex;
                align-items: center;
        ...
        <div class="<?php echo esc_attr( $uid ); ?> olo-woo-minicart-wrap">
            ...
            <span class="olo-mc-text olo-mc-total"><?php echo $total; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- price HTML generated by WooCommerce WC()->cart->get_cart_total() (escaped internally by wc_price) ?></span>
            <?php endif; ?>
            <?php endif; ?>
            <?php if ( ! empty( $s['dropdown'] ) ) : ?>
