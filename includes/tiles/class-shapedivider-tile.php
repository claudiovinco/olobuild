...
class Olobuild_Shapedivider_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        ob_start();
        ?>
        <?php // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above: intval()-clamped sizes, the 'top'/'bottom' position ternary, fixed scaleX/scaleY transform literals, a float offset computed from the clamped width and the internally generated $uid. ?>
        <style>
            .<?php echo $uid; ?> {
                position: absolute;
                <?php echo $position; ?>: 0;
        ...
        <div class="olo-shapedivider <?php echo esc_attr( $uid ); ?>">
            <svg preserveAspectRatio="none" viewBox="0 0 1200 120" xmlns="http://www.w3.org/2000/svg">
                <path d="<?php echo esc_attr( $path ); ?>" fill="<?php echo $color; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- colour validated by the safe_color_css() whitelist above; may legitimately be a var() token whose fallback contains quotes, which esc_attr() would re-encode ?>" />
            </svg>
        </div>
        <?php
                // Border system
