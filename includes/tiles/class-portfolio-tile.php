...
class Olo_Portfolio_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        if ( empty( $items ) ) {
            return '<div style="padding:40px;text-align:center;color:#9CA3AF;">' . esc_html( olo_t( 'Nessun elemento nel portfolio' ) ) . '</div>';
        }
        ...
        <?php if ( $preset_css ) echo '<style>' . $preset_css . '</style>'; ?>
            ...
            <?php if ( $layout === 'split-index' ) : ?>
                <div class="<?php echo esc_attr( $uid ); ?>-grid" id="<?php echo esc_attr( $uid ); ?>-grid">
                    <div class="olo-pf-list">
                        ...
                        ?>
                            <a href="<?php echo esc_url( $item['link'] ?: '#' ); ?>" class="olo-pf-item<?php echo $extra_class; ?>" data-categories="<?php echo esc_attr( $cat_data ); ?>" data-search="<?php echo esc_attr( strtolower( $item['title'] ) ); ?>" data-img="<?php echo esc_url( $item['image'] ); ?>" data-ribbon="<?php echo esc_attr( $s['featured_ribbon_text'] ); ?>" style="text-decoration:none;color:inherit">
                                <?php if ( ! empty( $s['index_numbering'] ) ) : ?><span class="olo-pf-index"><?php echo str_pad( $i + 1, 2, '0', STR_PAD_LEFT ); ?>/<?php echo str_pad( $total_items, 2, '0', STR_PAD_LEFT ); ?></span><?php endif; ?>
                    ...
                    <div class="olo-pf-preview">
                        <?php foreach ( $items as $i => $item ) : ?>
                            ...
                            <img src="<?php echo esc_url( $item['image'] ); ?>" alt="" data-i="<?php echo $i; ?>" class="<?php echo $i === 0 ? 'is-active' : ''; ?>" />
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </div>
                </div>
        ...
        (function(){
            ...
            <?php if ( ! empty( $s['enable_search'] ) ) : ?>
            var search = root.querySelector('.<?php echo $uid; ?>-search');
            if(search){
            ...
            <?php if ( ! empty( $s['cursor_label_enabled'] ) ) : ?>
            var cursor = root.querySelector('.<?php echo $uid; ?>-cursor');
            if(cursor){
    ...
    private function render_item( $item, $i, $total_items, $s, $fx, $caption_pos, $site_host ) {
        ...
        // Index numbering
        if ( ! empty( $s['index_numbering'] ) ) {
            echo '<span class="olo-pf-index">' . str_pad( $i + 1, 2, '0', STR_PAD_LEFT ) . '/' . str_pad( $total_items, 2, '0', STR_PAD_LEFT ) . '</span>';
        }
        ...
        if ( $caption_pos !== 'overlay' || $fx === 'color-splash' || $fx === 'glitch-rgb' || $fx === 'cinemagraph' || $fx === 'image-cycle' ) {
            ...
            if ( $show_text ) {
                ...
                if ( ! empty( $s['show_title'] ) ) {
                    ...
                    echo '<div class="olo-pf-title' . $pft_cls . '"' . $pft_data . '>' . esc_html( $item['title'] ) . '</div>';
                }
                if ( ! empty( $s['show_excerpt'] ) && ! empty( $item['description'] ) ) {
                    ...
                    echo '<div class="olo-pf-desc' . $pfd_cls . '"' . $pfd_data . '>' . wp_kses_post( $item['description'] ) . '</div>';
                }
                echo '</div>';
            }
        }
