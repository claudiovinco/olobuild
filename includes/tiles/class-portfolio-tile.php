...
class Olo_Portfolio_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        <?php if ( $preset_css ) echo '<style>' . $preset_css . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olo_Tile_Base::build_wow_effects_css() from sanitized wow_* settings ?>
        ...
        if ( $caption_pos !== 'overlay' || $fx === 'color-splash' || $fx === 'glitch-rgb' || $fx === 'cinemagraph' || $fx === 'image-cycle' ) {
            ...
            if ( $show_text ) {
                ...
                if ( ! empty( $s['show_title'] ) ) {
                    ...
                    echo '<div class="olo-pf-title' . $pft_cls . '"' . $pft_data . '>' . esc_html( $item['title'] ) . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- class/data attrs generated and escaped by Olo_Text_Effects helpers; title escaped with esc_html()
                }
                if ( ! empty( $s['show_excerpt'] ) && ! empty( $item['description'] ) ) {
                    ...
                    echo '<div class="olo-pf-desc' . $pfd_cls . '"' . $pfd_data . '>' . wp_kses_post( $item['description'] ) . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- attrs escaped by Olo_Text_Effects helpers; content sanitized via wp_kses_post()
                }
                echo '</div>';
            }
        }
