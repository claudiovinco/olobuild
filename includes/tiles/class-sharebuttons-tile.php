...
class Olobuild_Sharebuttons_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        ob_start();
        // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below only echoes the internally generated $uid; the hover colour is esc_attr()'d.
        ?>
        <style>
        ...
        <div id="<?php echo esc_attr( $uid ); ?>" class="olo-sharebuttons olo-sb-preset-<?php echo esc_attr( sanitize_key( $s['preset'] ?? 'custom' ) ); ?>" style="display:flex;flex-wrap:wrap;justify-content:<?php echo esc_attr( $justify ); ?>;gap:<?php echo (int) $gap; ?>px;padding:16px;">
            ...
            ?>
                <a href="#" class="olo-share-btn" data-olo-share="<?php echo esc_attr( $platform ); ?>"
                   style="display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:<?php echo $pad; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $pad/$w/$h are fixed literals or absint()'d size + 'px'; colours esc_attr()'d ?>;width:<?php echo $w; ?>;height:<?php echo $h; ?>;background:<?php echo esc_attr( $bg_clr ); ?>;color:<?php echo esc_attr( $icon_clr ); ?>;border-radius:8px;font-size:<?php echo (int) $icon_fs; ?>px;line-height:1;"
                   aria-label="<?php echo esc_attr( $label ); ?>">
                    <?php if ( $show_icon ) : ?>
                        <?php echo $icon_svg; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG markup from the hardcoded icon_svg() path map ?>
                    <?php endif; ?>
                    <?php if ( $show_text ) : ?>
