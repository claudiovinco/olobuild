...
class Olo_MegaMenu_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        if ( $border_css || $border_hover_css || $border_effect_css ) {
            echo '<style>';
            if ( $border_css ) echo ".{$uid}{{$border_css}}"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olo_Tile_Base::build_border_css() from intval'd widths and structured border settings; $uid is internally generated
            echo $border_hover_css . $border_effect_css . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olo_Tile_Base::build_border_hover_css()/build_border_effect_css() helpers
        }
        return ob_get_clean();
    }
    ...
    private function render_css( $s, $uid ) {
        ...
        // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above: colors via safe_color()/safe_color_css(), integers via intval()/absint()/floatval()/pad_int()/radius_int(), enums via fixed-literal ternaries/if-else branches, shadows via Olo_Tile_Utils fixed maps, font stacks via resolve_font_family() whitelist; $uid is internally generated.
        ?>
        <style>
    ...
    private function render_html( $tree, $children, $grandchildren, $s, $uid ) {
        ...
        <div class="olo-megamenu <?php echo esc_attr( $uid ); ?>" data-uid="<?php echo esc_attr( $uid ); ?>">
            ...
            <div class="olo-mm-bar">
                ...
                <?php if ( $mob_search ) : ?>
                <button class="olo-mm-mobile-search olo-mm-icon-btn" aria-label="<?php echo esc_attr( olo_t( 'Cerca' ) ); ?>" type="button"><?php echo $search_svg; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG markup from get_search_svg() (hardcoded strings) ?></button>
                <?php endif; ?>
                ...
                <a class="olo-mm-logo" href="<?php echo $logo_link; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- esc_url()-escaped at assignment above ?>">
                    <img class="olo-mm-logo-default" src="<?php echo $logo_img; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- esc_url()-escaped at assignment above ?>" alt="<?php echo esc_attr( olo_t( 'Logo' ) ); ?>">
                    <?php if ( $logo_sticky ) : ?><img class="olo-mm-logo-sticky" src="<?php echo $logo_sticky; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- esc_url()-escaped at assignment above ?>" alt="<?php echo esc_attr( olo_t( 'Logo' ) ); ?>" style="display:none;"><?php endif; ?>
                </a>
                ...
                ?>
                <button class="olo-mm-search-icon olo-mm-icon-btn" data-olo-search-style="<?php echo esc_attr( $search_style ); ?>" aria-label="<?php echo esc_attr( olo_t( 'Cerca' ) ); ?>" type="button"><?php echo $search_svg; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG markup from get_search_svg() (hardcoded strings) ?></button>
            ...
            <div class="olo-mm-mob-search-panel">
                <form action="<?php echo esc_url( home_url('/') ); ?>" method="get" role="search">
                    <input type="search" name="s" placeholder="<?php echo esc_attr( olo_t( 'Cerca...' ) ); ?>" autocomplete="off">
                    <button type="submit" aria-label="<?php echo esc_attr( olo_t( 'Cerca' ) ); ?>"><?php echo $search_svg; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG markup from get_search_svg() (hardcoded strings) ?></button>
                </form>
            </div>
            <?php endif; ?>
            ...
            <nav class="olo-mm-offcanvas" aria-label="<?php echo esc_attr( olo_t( 'Mobile menu' ) ); ?>">
                <div class="olo-mm-oc-header">
                    ...
                    <div class="olo-mm-oc-actions">
                        <?php if ( $mob_search ) : ?>
                        <button class="olo-mm-oc-search-btn olo-mm-icon-btn" type="button" aria-label="<?php echo esc_attr( olo_t( 'Cerca' ) ); ?>"><?php echo $search_svg; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG markup from get_search_svg() (hardcoded strings) ?></button>
                        <?php endif; ?>
