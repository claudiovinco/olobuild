...
class Olobuild_ProSlider_Tile extends Olobuild_Tile_Base {
        ...
        'slides'             => [
            [
                ...
                'background' => [
                    ...
                    'image'          => '',
                    'video'          => '',
                    'color'          => 'var(--olo-color-dark, #1e293b)',
                    'gradientFrom'   => 'var(--olo-color-dark, #1e293b)',
                    'gradientTo'     => 'var(--olo-color-dark, #0f172a)',
                    'gradientAngle'  => 180,
                    'kenBurns'       => false,
                    'kenBurnsScale'  => 1.2,
    ...
    public function render( $settings ) {
        ...
        if ( $google_fonts && class_exists( 'Olobuild_Font_Host' ) ) {
            // Self-hosted (Olobuild_Font_Host): nessuna richiesta del visitatore a Google.
            $ff_css = Olobuild_Font_Host::get_font_face_css( array_keys( $google_fonts ), '300;400;500;600;700;800;900' );
            if ( $ff_css ) {
        ...
        >
            ...
            if ( $s['showArrows'] && count( $slides ) > 1 ) : ?>
                <button class="olo-proslider-arrow olo-proslider-prev mps-arrow-<?php echo esc_attr( $arrow_style ); ?>" aria-label="<?php echo esc_attr( olobuild_t( 'Previous' ) ); ?>"><?php echo esc_html( olobuild_t( '&lsaquo;' ) ); ?></button>
                <button class="olo-proslider-arrow olo-proslider-next mps-arrow-<?php echo esc_attr( $arrow_style ); ?>" aria-label="<?php echo esc_attr( olobuild_t( 'Next' ) ); ?>"><?php echo esc_html( olobuild_t( '&rsaquo;' ) ); ?></button>
            <?php endif; ?>
        ...
        if ( ! empty( $s['showThumbs'] ) && count( $slides ) > 1 ) {
            $thumb_pos   = in_array( $s['thumbPosition'] ?? 'bottom', [ 'bottom', 'top', 'left', 'right' ], true ) ? ( $s['thumbPosition'] ?? 'bottom' ) : 'bottom';
        ...
        if ( ! empty( $s['showTabs'] ) && count( $slides ) > 1 ) {
            $tab_pos   = in_array( $s['tabPosition'] ?? 'bottom', [ 'bottom', 'top' ], true ) ? ( $s['tabPosition'] ?? 'bottom' ) : 'bottom';
        ...
        /*
         * Il bordo del tile si costruisce QUI, come in ogni altro tile.
         *
         * Questo blocco stava in fondo a render_layer(), dove ne' $s ne' $uid
         * esistono: leggeva null, quindi il bordo configurato dal docente non
         * compariva mai. Ed era anche il posto sbagliato per un altro motivo:
         * render_layer() gira una volta per ogni strato di ogni diapositiva,
         * quindi lo stesso <style> sarebbe stato ripetuto decine di volte.
         */
        ...
        if ( $border_css || $border_hover_css || $border_effect_css ) {
            ...
            echo $border_hover_css . $border_effect_css . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by the Olobuild_Tile_Base::build_border_hover_css()/build_border_effect_css() shared helpers
        }
    ...
    private function render_layer( $layer, $outside_wrap = false ) {
        ...
        if ( ! empty( $layer['hover'] ) && is_array( $layer['hover'] ) ) {
            ...
            $h_easing  = in_array( $h['easing'] ?? 'ease', ['ease','ease-in','ease-out','ease-in-out','linear'], true ) ? ( $h['easing'] ?? 'ease' ) : 'ease';
            // Transform
            $h_parts   = [];
        ...
        $bs = in_array( $layer['borderStyle'] ?? 'solid', $valid_border_styles, true ) ? ( $layer['borderStyle'] ?? 'solid' ) : 'solid';
    ...
    private function render_layer_content( $layer, $type ) {
        ...
        switch ( $type ) {
            case 'text':
                $tag     = in_array( $layer['tag'] ?? 'h2', [ 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'span', 'div' ], true ) ? ( $layer['tag'] ?? 'h2' ) : 'h2';
