    ...
    public function render( $settings ) {
        ...
        <figure class="olo-image <?php echo esc_attr( $uid ); ?>"<?php echo $align_data_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- attribute string built above with esc_attr() on a whitelisted value ?><?php if ( ! empty( $s['lightbox'] ) && empty( $s['link_url'] ) ) echo ' data-uk-lightbox'; ?> style="<?php echo esc_attr( $figure_style ); ?>">
            ...
            if ( ! empty( $s['link_url'] ) ) {
                ...
                if ( $parts['has'] ) {
                    echo '<div class="olo-img-media" style="position:relative;width:100%;height:100%;min-height:inherit;overflow:hidden;' . $parts['css'] . '">' . $parts['markup'] . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS and markup generated by Olo_Tile_Base::bg_media_parts()/Olo_CSS_Builder which sanitize internally.
                } else {
