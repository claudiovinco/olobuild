...
class Olobuild_Image_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        if ( $shadow_pref === 'custom' ) {
            ...
            $shadow_value = Olobuild_Tile_Utils::shadow( $shadow_pref );
        ...
        <figure class="olo-image <?php echo esc_attr( $uid ); ?>"<?php echo $align_data_attr; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- attribute string built above with esc_attr() on a whitelisted value ?><?php if ( ! empty( $s['lightbox'] ) && empty( $s['link_url'] ) ) echo ' data-uk-lightbox'; ?> style="<?php echo esc_attr( $figure_style ); ?>">
            ...
            $img    = Olobuild_Tile_Utils::img_srcset( $att_id, $s['image_url'], $s['alt_text'], 'uk-border-rounded', 'full', $extra, $img_opts );
            ...
            if ( ! empty( $s['link_url'] ) ) {
                ...
                // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- placeholders use esc_url/esc_attr; $link_rel/$link_title built with esc_attr() above; $img is markup generated by Olobuild_Tile_Utils::img_srcset()/render_hover_wrap() which escape internally.
                printf(
                ...
                // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- href uses esc_url(); $img is markup generated by Olobuild_Tile_Utils::img_srcset()/render_hover_wrap() which escape internally.
                printf(
                ...
                echo $img; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- markup generated by Olobuild_Tile_Utils::img_srcset()/render_hover_wrap() which escape internally.
            }
            else :
                ...
                if ( $parts['has'] ) {
                    echo '<div class="olo-img-media" style="position:relative;width:100%;' . $media_h_css . 'overflow:hidden;' . $parts['css'] . '">' . $parts['markup'] . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS and markup generated by Olobuild_Tile_Base::bg_media_parts()/Olobuild_CSS_Builder which sanitize internally.
                } else {
