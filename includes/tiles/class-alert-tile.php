...
class Olobuild_Alert_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        <div class="olo-alert uk-alert uk-alert-<?php echo esc_attr( $uk_type ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $alert_uid is the internally generated 'olo-alert-' . wp_unique_id(); $style_attr is assembled above from fixed literals, esc_attr()'d colours and an in_array() whitelisted alignment ?> <?php echo $alert_uid; ?> olo-alert-preset-<?php echo esc_attr( sanitize_key( $s['preset'] ?? 'custom' ) ); ?>"<?php echo $style_attr; ?> uk-alert>
            ...
            <div style="display: flex; align-items: flex-start; gap: 12px;">
                ...
                <div style="flex: 1;">
                    ...
                    <?php endif; ?>
                    <div class="olo-alert-msg<?php echo $m_tfx_cls; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- tfx_attrs() fragments are escaped internally (sanitize_html_class/esc_attr); message is esc_html()'d (nl2br only adds <br /> tags) ?>" style="font-size: 0.9em; line-height: 1.5;"<?php echo $m_tfx_data; ?>><?php echo nl2br( esc_html( $msg_plain ) ); ?></div>
                </div>
            </div>
        </div>
        <?php
        ...
        if ( $border_css || $border_hover_css || $border_effect_css ) {
            echo '<style>';
            if ( $border_css ) echo ".{$alert_uid}{{$border_css}}"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base::build_border_css() from sanitized settings; $alert_uid is internally generated
