...
class Olo_Woo_Addtocart_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        // Colors — TOKEN-FIRST: la CTA carrello eredita il brand se l'utente non sceglie.
        $bg_color   = $this->safe_color_css( $s['bg_color'] )   ?: 'var(--olo-color-primary, #e1474f)';
        ...
        <div class="<?php echo esc_attr( $uid ); ?>">
            ...
            <?php else : ?>
                ...
                >
                    ...
                    <?php echo $icon_svg; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG markup from the hardcoded $icon_svgs map above ?>
                    <?php endif; ?>
                    <span><?php echo $btn_text; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped via esc_html() at assignment above ?></span>
                </a>
            <?php endif; ?>
        </div>
        <?php
        ...
        <script>
        (function(){
            var wrap = document.querySelector('.<?php echo $uid; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- internal 'olo-woo-atc-' . wp_rand() identifier ?>');
            if(!wrap){return}
