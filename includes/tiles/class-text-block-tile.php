...
class Olo_TextBlock_Tile extends Olo_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        <div class="olo-text-block <?php echo $tb_uid; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $tb_uid is internally generated; tfx_attrs() fragments are escaped internally (sanitize_html_class/esc_attr) ?><?php echo $tb_cls; ?>" style="<?php echo esc_attr( $style ); ?>"<?php echo $tb_data; ?>>
            <?php echo $content; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- rich text sanitized via safe_richtext_content() (wp_kses_post) above, or esc_html()+nl2br for plain text ?>
        </div>
        <?php
        ...
        if ( $border_css || $border_hover_css || $border_effect_css || $hover_br_rule_css ) {
            echo '<style>';
            if ( $border_css ) echo ".{$tb_uid}{{$border_css}}"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olo_Tile_Base::build_border_css() from sanitized settings; $tb_uid is internally generated
            echo $border_hover_css . $border_effect_css . $hover_br_rule_css . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olo_Tile_Base border builders and build_border_radius_css() (intval-based) from sanitized settings
        }
        return ob_get_clean();
    }
}
