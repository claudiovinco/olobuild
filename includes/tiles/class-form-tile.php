...
class Olobuild_Form_Tile extends Olobuild_Tile_Base {
    ...
    public function render( $settings ) {
        ...
        <div class="olo-form <?php echo esc_attr( $uid ); ?> olo-fm-preset-<?php echo esc_attr( sanitize_key( $s['preset'] ?? 'custom' ) ); ?>"<?php if ( $container_style ) : ?> style="<?php echo esc_attr( $container_style ); ?>"<?php endif; ?>>
            ...
            $form_token = Olobuild_Form_Handler::generate_token( $config_b64 );
            ?>
            <form class="uk-form-stacked" data-olo-form="<?php echo esc_attr( $uid ); ?>" enctype="multipart/form-data">
                ...
                foreach ( $steps as $step_idx => $step_fields ) :
                    ...
                    <div class="uk-grid-small" uk-grid style="row-gap:<?php echo (int) $gap; ?>px">
                        <?php foreach ( $step_fields as $i => $field ) :
                            ...
                            // ID univoco del campo per associare <label for> all'input (a11y / EAA).
                            $field_id    = $uid . '-' . $fname;
                        ...
                        <div class="<?php echo esc_attr( $width_class . $cond_class ); ?>"<?php echo $cond_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- data-cond-* attribute string built above with esc_attr() around each value ?> data-field-name="<?php echo esc_attr( $fname ); ?>">
                            ...
                            <?php elseif ( $ftype === 'textarea' ) : ?>
                                <?php if ( $is_floating ) : ?>
                                    <div class="olo-f-float">
                                        <textarea id="<?php echo esc_attr( $field_id ); ?>" name="fields[<?php echo esc_attr( $fname ); ?>]" class="uk-textarea<?php echo esc_attr( $size_class ); ?>" rows="4" placeholder=" "<?php echo esc_attr( $req_attr ); ?>></textarea>
                                        <?php if ( $flabel ) : ?>
                                            <label for="<?php echo esc_attr( $field_id ); ?>" class="olo-f-float-label"><?php echo esc_html( $flabel ); ?><?php if ( $frequired ) : ?><span class="olo-f-required">*</span><?php endif; ?></label>
                                        <?php endif; ?>
                                    </div>
                                <?php else : ?>
                                    <?php if ( $flabel ) : ?>
                                        <label for="<?php echo esc_attr( $field_id ); ?>" class="olo-f-label"><?php echo esc_html( $flabel ); ?><?php if ( $frequired ) : ?><span class="olo-f-required">*</span><?php endif; ?></label>
                                    <?php endif; ?>
                                    <textarea id="<?php echo esc_attr( $field_id ); ?>" name="fields[<?php echo esc_attr( $fname ); ?>]" class="uk-textarea<?php echo esc_attr( $size_class ); ?>" rows="4" placeholder="<?php echo esc_attr( $fplaceholder ); ?>"<?php echo esc_attr( $req_attr ); ?>></textarea>
                                <?php endif; ?>
                            ...
                            <?php elseif ( $ftype === 'radio' ) : ?>
                                <fieldset class="olo-f-fieldset" style="border:0;padding:0;margin:0;min-width:0">
                                <?php if ( $flabel ) : ?>
                                    <legend class="olo-f-label" style="padding:0"><?php echo esc_html( $flabel ); ?><?php if ( $frequired ) : ?><span class="olo-f-required">*</span><?php endif; ?></legend>
                                <?php endif; ?>
                            ...
                            <?php elseif ( $ftype === 'file' ) : ?>
                                ...
                                <div class="olo-f-file-wrap" data-max-size="<?php echo (int) $f_max_size; ?>" data-max-files="<?php echo (int) $f_max_files; ?>" data-allowed="<?php echo $f_allowed; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- escaped via esc_attr() at assignment above ?>">
                                    ...
                                    <span class="olo-f-file-info" style="display:inline-block;margin-left:12px;font-size:13px;color:<?php echo $label_color; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- sanitized via safe_color_css() whitelist above ?>;opacity:0.7;"><?php echo esc_html( olobuild_t( 'Nessun file selezionato' ) ); ?></span>
                            ...
                            <?php elseif ( $ftype === 'password_confirm' ) : ?>
                                ...
                                <div style="display:flex;flex-direction:column;gap:8px">
                                    <div class="uk-inline uk-width-1-1">
                                        <input type="password" id="<?php echo esc_attr( $field_id ); ?>" name="fields[<?php echo esc_attr( $fname ); ?>]" class="uk-input<?php echo $size_class; ?>" placeholder="<?php echo esc_attr( $fplaceholder ?: 'Password' ); ?>"<?php echo $req_attr; ?> autocomplete="new-password" data-pwc-main="<?php echo $pc_id; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- id composed of literal prefix, esc_attr()-escaped name and wp_rand(); required/size-class are fixed internal strings ?>" />
                                        <a class="uk-form-icon uk-form-icon-flip" style="cursor:pointer" onclick="var i=this.previousElementSibling;i.type=i.type==='password'?'text':'password'" uk-icon="icon: eye"></a>
                                    </div>
                                    <div class="uk-inline uk-width-1-1">
                                        <input type="password" class="uk-input<?php echo $size_class; ?>" placeholder="<?php echo esc_attr( olobuild_t( 'Conferma password' ) ); ?>" autocomplete="new-password" data-pwc-confirm="<?php echo $pc_id; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- id composed of literal prefix, esc_attr()-escaped name and wp_rand(); size-class is a fixed internal string ?>" />
                                        <a class="uk-form-icon uk-form-icon-flip" style="cursor:pointer" onclick="var i=this.previousElementSibling;i.type=i.type==='password'?'text':'password'" uk-icon="icon: eye"></a>
                                    </div>
                    ...
                    <?php if ( $is_multistep ) : ?>
                        <div class="olo-f-step-nav">
                            <?php if ( $step_idx > 0 ) : ?>
                                <button type="button" class="olo-f-btn olo-f-step-prev"><?php echo esc_html( olobuild_t( 'Indietro' ) ); ?></button>
                            <?php endif; ?>
                            <?php if ( $step_idx < $step_count - 1 ) : ?>
                                <button type="button" class="olo-f-btn olo-f-step-next"><?php echo esc_html( olobuild_t( 'Avanti' ) ); ?></button>
                            <?php endif; ?>
                        </div>
                ...
                <div class="olo-f-msg olo-f-msg--success" role="status" aria-live="polite" data-anim="<?php echo esc_attr( $succ_anim ); ?>"></div>
                <div class="olo-f-msg olo-f-msg--error" role="alert" aria-live="assertive" data-anim="<?php echo esc_attr( $err_anim ); ?>"></div>
            </form>
        </div>

        <?php
        if ( $has_recaptcha ) {
            // reCAPTCHA v3 caricato via enqueue (stampato nel footer della pagina).
            ...
            wp_enqueue_script( 'olo-recaptcha', 'https://www.google.com/recaptcha/api.js?render=' . rawurlencode( $recaptcha_site_key ), [], null, true );
        }
        ?>
