...
class Olo_Countdown_Tile extends Olo_Tile_Base {
    ...
    private function render_uikit( $s ) {
        ...
        ob_start();
        ?>
        <?php // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- inline CSS below is built exclusively from values sanitized above: absint() for every size/weight, safe_color_css() whitelist for every colour, Olo_Tile_Utils::spacing_css()/build_border_radius_css() (integer-forced); uid is generated internally. ?>
        <style>
            .<?php echo $uid; ?> {
