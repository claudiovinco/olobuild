...
class Olobuild_IconTabs_Tile extends Olobuild_Tile_Base {
    ...
    protected $defaults = [
        ...
        'active_color'   => '',
        'inactive_color' => '',
        'card_bg'        => 'var(--olo-color-surface-alt, #F9D7D7)',
        'card_radius'    => '16',
        'effect_color'   => '',
    ...
    public function render( $settings ) {
        ...
        $radius        = max( 0, Olobuild_Tile_Utils::radius_int( $s['card_radius'] ) );
        ...
        // Raggio card in hover: DOPO il bordo in hover, di cui riprende la transizione
        // (una sola `transition` vince: così si animano entrambi).
        $card_radius_css = Olobuild_Tile_Utils::radius_hover_rules( $card_border_sel, $s, 'card_radius_hover', Olobuild_Tile_Utils::transizione_di( $card_border_hover_css ) );

        if ( $border_css || $border_hover_css || $border_effect_css || $card_border_css || $card_border_hover_css || $card_radius_css ) {
            echo '<style>';
            ...
            echo $border_hover_css . $border_effect_css; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base border helpers from sanitized border settings
            if ( $card_border_css ) echo "{$card_border_sel}{{$card_border_css}}"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base::build_border_css() from sanitized card border settings; selector from internal uid
            echo $card_border_hover_css . $card_radius_css . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by Olobuild_Tile_Base::build_border_hover_css() and Olobuild_Tile_Utils::radius_hover_rules() (integer px) from sanitized card settings
        }
        return ob_get_clean();
    }
}
